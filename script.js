...
// BLOK 1: INISIALISASI, DATA MASTER & WAKTU
// ==========================================
if ('serviceWorker' in navigator) {
    window.addEventListener('load', async () => {
        try {
            ...
            if (worker) {
                worker.postMessage({ type: 'CACHE_OFFLINE_ASSETS', timestamp: Date.now() });
                console.log('📦 Pesan caching dikirim ke Service Worker');
            }
        } catch (error) { console.warn('⚠️ Service Worker tidak terdaftar:', error); }
    });
}
...
const liburSpesifik = ['2026-08-17', '2026-09-25'];
const rentangLibur = [
    { mulai: '2026-07-01', selesai: '2026-07-31' },
    { mulai: '2026-08-01', selesai: '2026-08-09' },
    { mulai: '2026-12-21', selesai: '2027-01-03' },
    { mulai: '2027-02-08', selesai: '2027-02-09' },
    { mulai: '2027-02-22', selesai: '2027-03-20' }
];
const rentangKegiatan = [
    { mulai: '2026-09-14', selesai: '2026-09-23', nama: 'Sumatif Tengah Semester (STS)' },
    { mulai: '2026-10-05', selesai: '2026-10-07', nama: 'Audisi Festival Internal Abu Hurairah' },
...
let GAS_URL = localStorage.getItem('GAS_URL') || 'https://script.google.com/macros/s/AKfycbwTPzLaw_wwbvVu-GzbUiUkK9jIqh7d5N_BXY9PJrLk3jk-3qvsRgoBiFOtTcaIHmtC/exec';
const DB_NAME = 'AbsensiGuruDB';
const STORE_NAME = 'attendance';
let db = null;
let serverDataCache = [];
...
function getStatusHariIni(dateObj) {
    ...
    if (liburSpesifik.includes(tanggalFormat)) return { status: 'libur', label: 'Libur Spesifik / Tanggal Merah' };
    for (let r of rentangLibur) {
        if (tanggalFormat >= r.mulai && tanggalFormat <= r.selesai) return { status: 'libur', label: 'Libur Sekolah / Ramadhan' };
    }
    for (let r of rentangKegiatan) {
        if (tanggalFormat >= r.mulai && tanggalFormat <= r.selesai) return { status: 'kegiatan', label: r.nama };
    }
    return { status: 'normal', label: '' };
}
...
function getDayName() { return ['ahad', 'senin', 'selasa', 'rabu', 'kamis', 'jumat', 'sabtu'][getCurrentWITA().getDay()]; }
function getDayNameID() { return ['Ahad', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'][getCurrentWITA().getDay()]; }
...
function formatKopelClass(kelas, mapel) {
    ...
    if (group) return group.map(c => c === kelas ? `(${c})` : c).join(',');
    return kelas;
}
...
function getDaftarSantriKopel(baseClass, subject) {
    ...
    if (subjectLower.includes('tahfiz')) {
        ...
        for (let group of kopelTahfizh) { if (group.includes(baseClass)) { classesToFetch = group; break; } }
    } else if (subjectLower.includes('pjok') || subjectLower.includes('olahraga')) {
        const kopelPJOK = [['9A', '9B'], ['9C', '9D'], ['7D', '7E'], ['7A', '7B'], ['8A', '8B'], ['7C', '8E']];
        for (let group of kopelPJOK) { if (group.includes(baseClass)) { classesToFetch = group; break; } }
    }

    let allSantri = [];
    classesToFetch.forEach(cls => { if (DATA_SANTRI[cls]) allSantri = allSantri.concat(DATA_SANTRI[cls]); });
    return allSantri;
}
...
function getCurrentJPInfo(mode = '') {
    ...
    for (let jp in jadwal) {
        ...
        let k = parseInt(kArr[0], 10)*60 + parseInt(kArr[1], 10);
        let isValid = false;
        let allowEarly = jp === 'Pembiasaan Pagi' ? 30 : 10;
        
        if (mode === 'masuk') { isValid = (totalMins >= (m - allowEarly) && totalMins <= k); } 
        else if (mode === 'keluar') { isValid = (totalMins >= m && totalMins <= (k + 20)); } 
        else { isValid = (totalMins >= (m - allowEarly) && totalMins <= (k + 20)); }
        ...
        for (let c of CLASSES) {
            ...
            if (mapel) {
                ...
                if (isSequential) {
                    ...
                    let jpsPush = jp === 'Pembiasaan Pagi' ? ['Pembiasaan Pagi'] : [parseInt(jp.replace('JP ', ''))];
                    currentBlock = {
                        class: formattedClass, mapel: mapel, jps: jpsPush, jpStr: jp === 'Pembiasaan Pagi' ? 'Pembiasaan Pagi' : jp,
                        masuk: jadwal[jp].masuk, keluar: jadwal[jp].keluar, kMins: k
                    };
                }
                break;
...
function getNextClass() {
    ...
    for (let b of blocks) {
        let mMins = parseTimeToMins(b.masuk);
        if (mMins > totalMins) { return { jp: b.jpStr, class: b.class, mapel: b.mapel, masuk: b.masuk, masukMins: mMins }; }
    }
    return null;
}

// ==========================================
// BLOK 2: DATABASE, API SINKRONISASI & UI AUTH
// ==========================================
function openDB() {
    return new Promise((resolve, reject) => {
        try {
        ...
        try {
            ...
            if (item.action === 'masuk') {
                ...
                reqAll.onsuccess = () => {
                    const records = reqAll.result || [];
                    records.forEach(r => {
                        if (r.action === 'masuk' && r.class === item.class && r.date === item.date && r.jp === item.jp) store.delete(r.id);
                    });
                    resolve();
                };
                reqAll.onerror = () => reject(reqAll.error);
            } else { store.delete(item.id); resolve(); }
        } catch (e) { reject(e); }
    });
}
...
async function sendToGoogleSheet(record) {
    try {
        const payload = {
            ...
            action: record.action || '', date: record.date || '', time: record.time || '',
            day: record.day || '', jp: record.jp || '', timestamp: record.timestamp || '',
            jadwal_masuk: record.jadwal_masuk || '', jadwal_keluar: record.jadwal_keluar || '', keterangan: record.keterangan || ''
        };
        const response = await fetch(GAS_URL, {
            method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' },
            body: JSON.stringify({ api_action: 'addAttendance', ...payload })
...
async function syncNow(showToastMsg = true, showLog = false) {
    ...
    try {
        ...
        if (unsynced.length === 0) {
            if (showLog) addSyncLog('✅ Tidak ada data pending.', 'success');
            if (showToastMsg) showToast('✅ Tidak ada data pending.', 'success');
            updateSyncStatus(); await refreshServerCache(); renderMySchedule();
            return { success: true, message: 'Tidak ada data' };
        }
        if (showLog) addSyncLog(`📤 Pushing ${unsynced.length} data...`, 'info');
        if (showToastMsg) showToast(`🔄 Pushing ${unsynced.length} data...`, 'info');
        ...
        for (const item of unsynced) {
            ...
            if (!fresh || fresh.synced !== false) continue;
            const all = await getLocalHistory(fresh.teacher, fresh.date).catch(() => []);
            const sameLocal = all.some(r => r.id !== fresh.id && r.action === fresh.action && r.class === fresh.class && r.jp === fresh.jp && r.subject === fresh.subject && r.synced === true);
            if (sameLocal) { await processSyncedItem(fresh); continue; }
            
            const result = await sendToGoogleSheet(fresh);
            if (result.success) { successCount++; await processSyncedItem(fresh); if (showLog) addSyncLog(`✅ Item ${fresh.id} terkirim`, 'success'); } 
            else { failCount++; if (showLog) addSyncLog(`❌ Item ${fresh.id} gagal: ${result.error}`, 'error'); }
        }
        await refreshServerCache(); updateSyncStatus(); loadHistory(); loadRekap();
        const msg = `Sync selesai: ${successCount} berhasil, ${failCount} gagal.`;
        if (showLog) addSyncLog((failCount ? '⚠️ ' : '✅ ') + msg, failCount ? 'error' : 'success');
        if (showToastMsg) showToast((failCount ? '⚠️ ' : '✅ ') + msg, failCount ? 'warning' : 'success');
        return { success: failCount === 0, message: msg };
...
async function doLogin(name, isAdminUser, adminType) {
    ...
    currentMode = null; updateScanModeUI(); stopScanner();
    updateClock(); updateOfflineStatus();
    updateNextClassInfo(); checkActiveTeachingForIzinKasus();
    
    if (navigator.onLine) { await refreshServerCache(); syncNow(false, true); }
    
    updateSyncStatus(); loadHistory(); 
    showToast(`Assalamu'alaikum ustadz ${name}`, 'success');
    
    localStorage.setItem('absensi_user', name);
    if (isAdminUser) { localStorage.setItem('absensi_admin', 'true'); localStorage.setItem('absensi_admin_type', adminType || ''); } 
    else { localStorage.removeItem('absensi_admin'); localStorage.removeItem('absensi_admin_type'); }
}
...
// ==========================================
// BLOK 3: PEMINDAI QR & LOGIKA ABSENSI INTI
// ==========================================
function updateScanModeUI() {
    let activeTeaching = JSON.parse(localStorage.getItem('activeTeaching_' + currentUser) || "null");
    const btnMasuk = document.getElementById('modeMasuk');
    ...
    let masukClass = 'btn btn-sm btn-outline';
    let keluarClass = 'btn btn-sm btn-outline';
    let isMasukAnimated = false, isKeluarAnimated = false;
    let isPastEndTime = false, isNearEndTime = false;

    if (activeTeaching && activeTeaching.keluar) {
        const endParts = activeTeaching.keluar.split(':');
        const keluarMins = parseInt(endParts[0]) * 60 + parseInt(endParts[1]);
        const todayStr = getLocalDateString(now);
        if (activeTeaching.date && activeTeaching.date !== todayStr) { isPastEndTime = true; } 
        else if (currentMins >= keluarMins) { isPastEndTime = true; } 
        else if ((keluarMins - currentMins) <= 5 && (keluarMins - currentMins) >= 0) { isNearEndTime = true; }
    }

    if (activeTeaching && !isPastEndTime) { currentMode = 'keluar'; } 
    else if (!activeTeaching) { if (currentMode !== 'masuk' && currentMode !== 'keluar') currentMode = 'masuk'; }
    ...
    if (activeTeaching) {
        if (activeTeaching.keluarScanned) {
            ...
            keluarText = `<i class="fa-solid fa-door-open"></i> Scan Keluar Kelas ${activeTeaching.class}`;
            if (isPastEndTime || isNearEndTime) {
                isKeluarAnimated = true; keluarClass = 'btn btn-sm'; 
                if (isNextClassNear) {
                    masukText = `<i class="fa-solid fa-arrow-right-to-bracket"></i> Scan Masuk Kelas ${nextClass.class}`;
                    masukClass = currentMode === 'masuk' ? 'btn btn-sm btn-primary' : 'btn btn-sm btn-outline';
                    document.getElementById('scanInstruction').innerHTML = `Fokus scan <b>KELUAR</b> terlebih dahulu sebelum menuju kelas ${nextClass.class}.`;
        ...
        if (isNextClassNear) {
            masukText = `<i class="fa-solid fa-arrow-right-to-bracket"></i> Scan Masuk Kelas ${nextClass.class} pukul ${nextClass.masuk}`;
            isMasukAnimated = true;
            if (currentMode === 'keluar') document.getElementById('scanInstruction').innerHTML = `Arahkan kamera ke QR Code kelas untuk absen <b>KELUAR</b> (Jika Anda lupa absen masuk).`;
            else document.getElementById('scanInstruction').innerHTML = `Kelas <b>${nextClass.class}</b> akan segera dimulai. Silakan scan masuk!`;
        } else {
            if (currentMode === 'keluar') document.getElementById('scanInstruction').innerHTML = `Arahkan kamera ke QR Code kelas untuk absen <b>KELUAR</b> (Jika Anda lupa absen masuk).`;
            else document.getElementById('scanInstruction').textContent = 'Arahkan kamera ke QR Code kelas untuk absen MASUK';
        }
        masukClass = currentMode === 'masuk' ? 'btn btn-sm btn-primary' : 'btn btn-sm btn-outline'; 
        keluarClass = currentMode === 'keluar' ? 'btn btn-sm btn-primary' : 'btn btn-sm btn-outline';
    }

    if (currentMode) document.getElementById('scanModeBadge').innerHTML = currentMode === 'masuk' ? '<i class="fa-solid fa-arrow-right-to-bracket"></i> Masuk' : '<i class="fa-solid fa-door-open"></i> Keluar';

    btnMasuk.classList.remove('btn-pulse-red'); btnKeluar.classList.remove('btn-pulse-red');
    btnMasuk.innerHTML = masukText; btnKeluar.innerHTML = keluarText;
    btnMasuk.className = masukClass; btnKeluar.className = keluarClass;

    if (isMasukAnimated) { btnMasuk.classList.add('btn-pulse-red'); if (currentMode === 'masuk') btnMasuk.classList.remove('btn-outline'); }
    if (isKeluarAnimated) { btnKeluar.classList.add('btn-pulse-red'); if (currentMode !== 'keluar') { currentMode = 'keluar'; btnKeluar.classList.remove('btn-outline'); } }
}
...
window.editKeterangan = async function(encodedRecord) {
    if (!navigator.onLine) { showToast('Anda harus terkoneksi internet (Online) untuk mengedit alasan ke server.', 'error'); return; }
    const record = JSON.parse(decodeURIComponent(encodedRecord));
    let currentReason = record.keterangan || '';
    if (currentReason.startsWith('Telat: ')) currentReason = currentReason.substring(7);
    ...
    showLateReasonModal(async (newReason) => {
        ...
        try {
            ...
            if (record.id) {
                const getReq = store.get(record.id);
                getReq.onsuccess = () => { let data = getReq.result; if (data) { data.keterangan = finalKet; store.put(data); } };
            }
        } catch(e) {}
        showToast('Mengirim perubahan ke server...', 'info');
        try {
            const payload = { api_action: 'editKeterangan', teacher: record.teacher, date: record.date, class: record.class, jp: record.jp, action: record.action, keterangan: finalKet };
            const response = await fetch(GAS_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify(payload) });
            const res = await response.json();
            if (res.success) { showToast('✅ Alasan berhasil diperbarui!', 'success'); await refreshServerCache(); loadHistory(); } 
            else { throw new Error(res.error); }
        } catch(e) { showToast('❌ Gagal update ke server: ' + e.message, 'error'); }
    }, currentReason);
};
...
function startScanner() {
    ...
    html5QrCode = new Html5Qrcode("reader");
    html5QrCode.start({ facingMode: "environment" }, { fps: 10, qrbox: { width: 220, height: 220 } },
        (text) => { if (scanInProgress) return; stopScanner(); processScan(text.trim().toUpperCase()); }, () => {}
    ).then(() => isScanning = true).catch((err) => {
        console.error("Scanner initialization failed:", err);
        showToast('Error kamera/scan. Membersihkan cache...', 'error');
        clearAppCacheAndReload(); 
...
function buildScanFingerprint(classCode, action, teacher, date, jp, subject) {
    return [teacher, date, action, classCode, jp, subject].map(v => String(v || '').trim().toUpperCase()).join('|');
}

async function processScan(classCode) {
    if (scanInProgress) { showScanResult('⏳ Scan sebelumnya masih diproses. Tunggu sebentar...', 'warning'); return; }
    if (!CLASSES.includes(classCode)) { showScanResult('❌ Kelas invalid: ' + classCode, 'error'); return; }
    if (!currentUser) { showScanResult('Silakan login.', 'error'); return; }
    ...
    for (let i = 1; i <= 8; i++) {
        ...
        let mapel = getMapelForScan(classCode, currentUser, day, jp);
        if (mapel) {
            let m = parseTimeToMins(jadwal[jp].masuk); let k = parseTimeToMins(jadwal[jp].keluar);
            if (!currentBlock) { currentBlock = { jps: [jp], mapel: mapel, masuk: m, keluar: k, strMasuk: jadwal[jp].masuk, strKeluar: jadwal[jp].keluar }; } 
            else {
                if (currentBlock.mapel === mapel) { currentBlock.jps.push(jp); currentBlock.keluar = k; currentBlock.strKeluar = jadwal[jp].keluar; } 
                else { myBlocks.push(currentBlock); currentBlock = { jps: [jp], mapel: mapel, masuk: m, keluar: k, strMasuk: jadwal[jp].masuk, strKeluar: jadwal[jp].keluar }; }
            }
        } else { if (currentBlock) { myBlocks.push(currentBlock); currentBlock = null; } }
    }
    if (currentBlock) myBlocks.push(currentBlock);

    let activeBlock = null; let minDiff = 9999;
    for (let i = 0; i < myBlocks.length; i++) {
        let b = myBlocks[i]; let m = b.masuk; let k = b.keluar; let isValid = false;
        let allowEarlyMins = b.jps.includes('Pembiasaan Pagi') ? 30 : 10; 
        
        if (currentMode === 'masuk') { isValid = (timeMins >= (m - allowEarlyMins) && timeMins <= k); } 
        else if (currentMode === 'keluar') { isValid = (timeMins >= m && timeMins <= (k + 20)); } 
        else { isValid = (timeMins >= (m - allowEarlyMins) && timeMins <= (k + 20)); }
        
        if (isValid) { let center = (m + k) / 2; let diff = Math.abs(timeMins - center); if (diff < minDiff) { minDiff = diff; activeBlock = b; } }
    }
    ...
    if (currentMode === 'keluar' && activeTeaching && activeTeaching.rawClass === classCode) {
        jpFinal = activeTeaching.jp; mapelFinal = activeTeaching.subject; jMasuk = activeTeaching.masuk; jKeluar = activeTeaching.keluar;
    } else {
        if (activeBlock) {
            let start = activeBlock.jps[0].replace('JP ', ''); let end = activeBlock.jps[activeBlock.jps.length-1].replace('JP ', '');
            jpFinal = activeBlock.jps.length > 1 ? `JP ${start}-${end}` : activeBlock.jps[0];
            mapelFinal = activeBlock.mapel; jMasuk = activeBlock.strMasuk; jKeluar = activeBlock.strKeluar;
        } else {
            jpFinal = getCurrentJPInfo(currentMode); mapelFinal = getMapelForScan(classCode, currentUser, day, jpFinal);
            let bWaktu = jadwal[jpFinal] || { masuk: '00:00', keluar: '00:00' }; jMasuk = bWaktu.masuk; jKeluar = bWaktu.keluar;
        }
    }
    ...
    if (currentMode === 'keluar' && !activeTeaching) {
        ...
        let isMasukFoundServer = serverDataCache.some(h => h.teacher === currentUser && h.action === 'masuk' && h.class === formatKopelClass(classCode, mapelFinal) && h.jp === jpFinal && h.date === dateStr);
        if (!isMasukFoundLocally && !isMasukFoundServer) {
            showScanResult(`⚠️ Ditolak: Anda belum absen MASUK untuk kelas ini pada sesi ${jpFinal}.`, 'error'); playBeepWarning(); return;
        }
    }
    ...
    const tolMasuk = parseInt(localStorage.getItem('tolMasuk')) || 3;
    let isLate = false; let m = parseTimeToMins(jMasuk);
    if (currentMode === 'masuk' && m > 0) {
        let toleranceLimit = (jpFinal === 'Pembiasaan Pagi') ? 3 : tolMasuk;
        if ((timeMins - m) > toleranceLimit) isLate = true;
    }
...
async function executeFinalScanRecord(classCode, ket, subject, jpFinal, jMasuk, jKeluar, dateStr, timeStr, day, now) {
    if (scanInProgress) { showScanResult('⏳ Scan sedang diproses. Jangan scan ulang.', 'warning'); return; }
    scanInProgress = true;
    ...
    try {
        ...
        const fingerprint = buildScanFingerprint(classCode, action, currentUser, dateStr, jpFinal, subject);
        const nowMs = Date.now();
        if (fingerprint === lastScanFingerprint && (nowMs - lastScanAt) < 15000) { showScanResult('⚠️ QR yang sama baru saja diproses. Scan diabaikan.', 'warning'); return; }
        lastScanFingerprint = fingerprint; lastScanAt = nowMs;

        let activeTeaching = JSON.parse(localStorage.getItem('activeTeaching_' + currentUser) || "null");

        if (action === 'masuk' && activeTeaching && !activeTeaching.keluarScanned) {
            const oldClass = activeTeaching.class; const oldSubject = activeTeaching.subject; const oldJp = activeTeaching.jp || '-';
            const oldHist = await getLocalHistory(currentUser, dateStr).catch(() => []);
            const oldAlreadyOutLocal = oldHist.some(r => r.action === 'keluar' && r.class === oldClass && r.jp === oldJp);
            const oldAlreadyOutServer = serverDataCache.some(r => r.teacher === currentUser && r.action === 'keluar' && r.class === oldClass && r.jp === oldJp && r.date === dateStr);

            if (!oldAlreadyOutLocal && !oldAlreadyOutServer) {
                const recordLupa = {
                    teacher: currentUser, class: oldClass, subject: oldSubject, action: 'keluar', date: activeTeaching.date || dateStr,
                    time: timeStr, day: getDayNameID(), jp: oldJp, jadwal_masuk: activeTeaching.masuk || '-', jadwal_keluar: activeTeaching.keluar || '-',
                    timestamp: now.toISOString(), keterangan: 'lupa absen keluar', synced: false
                };
                const oldId = await saveAttendanceLocal(recordLupa); recordLupa.id = oldId; addHistoryRow({...recordLupa, id: oldId});
                let oldSent = false;
                if (navigator.onLine) {
                    const oldRes = await sendToGoogleSheet(recordLupa); oldSent = !!oldRes.success;
                    if (oldSent) await processSyncedItem(recordLupa);
                }
                if (oldSent) { activeTeaching.keluarScanned = true; localStorage.setItem('activeTeaching_' + currentUser, JSON.stringify(activeTeaching)); } 
                else if (!navigator.onLine) { showToast(`📱 Kelas ${oldClass} ditutup lokal; akan disinkronkan saat online.`, 'warning'); } 
                else { showToast(`⚠️ Auto-out kelas ${oldClass} gagal dikirim. Data tetap disimpan lokal.`, 'warning'); }
            }
        }

        const localHist = await getLocalHistory(currentUser, dateStr).catch(() => []);
        const duplicateLocal = localHist.some(h => h.action === action && h.class === formattedClass && h.jp === jpFinal && h.subject === subject);
        const duplicateServer = serverDataCache.some(h => h.teacher === currentUser && h.action === action && h.class === formattedClass && h.jp === jpFinal && h.subject === subject && h.date === dateStr);
        if (duplicateLocal || duplicateServer) { showScanResult(`⚠️ Anda sudah melakukan absen ${action.toUpperCase()} untuk ${formattedClass} / ${jpFinal}.`, 'warning'); playBeepWarning(); document.getElementById('keteranganInput').value = ''; return; }

        if (action === 'keluar') {
            const hasMatchingIn = localHist.some(h => h.action === 'masuk' && h.class === formattedClass && h.jp === jpFinal && h.date === dateStr) ||
                serverDataCache.some(h => h.teacher === currentUser && h.action === 'masuk' && h.class === formattedClass && h.jp === jpFinal && h.date === dateStr);
            if (!hasMatchingIn) { showScanResult(`⚠️ Scan KELUAR ditolak: data MASUK ${formattedClass} / ${jpFinal} tidak ditemukan.`, 'error'); playBeepWarning(); return; }
        }

        if (action === 'masuk' && jKeluar !== '00:00') {
            const activeTeachingData = { class: formattedClass, subject: subject, jp: jpFinal, masuk: jMasuk, keluar: jKeluar, notified: false, rawClass: classCode, keluarScanned: false, date: dateStr };
            localStorage.setItem('activeTeaching_' + currentUser, JSON.stringify(activeTeachingData)); updateNextClassInfo(); checkActiveTeachingForIzinKasus();
        }

        const record = { teacher: currentUser, class: formattedClass, subject: subject, action: action, date: dateStr, time: timeStr, day: getDayNameID(), jp: jpFinal, jadwal_masuk: jMasuk, jadwal_keluar: jKeluar, timestamp: now.toISOString(), keterangan: ket, synced: false };
        const id = await saveAttendanceLocal(record); record.id = id;
        showScanResult(ket ? `${action === 'masuk' ? '✅ In' : '🚪 Out'} ${formattedClass} (${ket})` : `${action === 'masuk' ? '✅ In' : '🚪 Out'} ${formattedClass} (${subject})`, 'success');
        addHistoryRow({...record, id});

        if (navigator.onLine) {
            const res = await sendToGoogleSheet(record);
            if (res.success) {
                await processSyncedItem(record); await refreshServerCache();
                showScanResult('✅ Data terkirim ke Server dan tersimpan sekali.', 'success');
                if (action === 'keluar') {
                    const actTeach = JSON.parse(localStorage.getItem('activeTeaching_' + currentUser) || "null");
                    if (actTeach && actTeach.class === formattedClass && actTeach.jp === jpFinal) {
                        localStorage.removeItem('activeTeaching_' + currentUser);
                        const timerCard = document.getElementById('activeClassTimerCard'); if (timerCard) timerCard.classList.add('hidden');
                        checkActiveTeachingForIzinKasus(); currentMode = null; updateScanModeUI();
                    }
                }
            } else { showScanResult('⚠️ Data tersimpan lokal. Gagal kirim ke server; tidak dibuat kiriman kedua.', 'warning'); }
        } else { showScanResult('📱 Offline: data tersimpan lokal dan akan dikirim saat online.', 'info'); }
        updateSyncStatus(); playBeep(); document.getElementById('keteranganInput').value = ''; renderMySchedule();
    } catch (err) {
        console.error('executeFinalScanRecord error:', err);
        showScanResult('❌ Terjadi kesalahan saat memproses absensi: ' + (err.message || err), 'error');
        clearAppCacheAndReload();
    } finally { setTimeout(() => { scanInProgress = false; }, 500); }
}
...
// ==========================================
// BLOK 4: AUDIO, ALARM, TIMER & RENDER UI
// ==========================================
window.lastKnownJP = undefined; window.isAlarmMuted = false; window.continuousAlarmInterval = null; window.nextClassNotifiedTeaching = false;
...
const audioRing = new Audio('ring.mp3'); const audioAlarm = new Audio('alarm.mp3');
let isAudioUnlocked = false;

function unlockAudio() {
    if (isAudioUnlocked) return;
    audioRing.play().then(() => { audioRing.pause(); audioRing.currentTime = 0; }).catch(e => {});
    audioAlarm.play().then(() => { audioAlarm.pause(); audioAlarm.currentTime = 0; }).catch(e => {});
    isAudioUnlocked = true;
...
function playJpChangeAlarm() { audioRing.currentTime = 0; audioRing.play().catch(e => console.log("Audio tertahan oleh browser:", e)); }
function playContinuousBeep() { audioAlarm.currentTime = 0; audioAlarm.play().catch(e => console.log("Audio tertahan oleh browser:", e)); }
function playShortBeep3x() { audioAlarm.currentTime = 0; audioAlarm.play().catch(e => console.log("Audio tertahan oleh browser:", e)); }
function playBeep() { try { const ctx = new(window.AudioContext || window.webkitAudioContext)(); const osc = ctx.createOscillator(); const gain = ctx.createGain(); osc.connect(gain); gain.connect(ctx.destination); osc.frequency.value = 1000; osc.type = 'square'; gain.gain.value = 0.5; osc.start(); setTimeout(() => { osc.stop(); ctx.close(); }, 150); } catch (e) {} }
function playBeep3x() { let count = 0; let intv = setInterval(() => { playBeep(); count++; if(count >= 3) clearInterval(intv); }, 800); }
function playBeepWarning() { let count = 0; let intv = setInterval(() => { try { const ctx = new(window.AudioContext || window.webkitAudioContext)(); const osc = ctx.createOscillator(); const gain = ctx.createGain(); osc.connect(gain); gain.connect(ctx.destination); osc.frequency.value = 800; osc.type = 'triangle'; gain.gain.value = 0.5; osc.start(); setTimeout(() => { osc.stop(); ctx.close(); }, 200); } catch(e) {} count++; if(count >= 3) clearInterval(intv); }, 400); }
...
window.stopContinuousAlarm = function() {
    if(window.continuousAlarmInterval) clearInterval(window.continuousAlarmInterval);
    window.continuousAlarmInterval = null; window.isAlarmMuted = true;
    document.getElementById('muteAlarmBtn').classList.add('hidden');
}

function updateNextClassInfo() {
    if (!currentUser) return;
    const now = getCurrentWITA(); const hour = now.getHours();
    const infoCard = document.getElementById('nextClassCard'); const warnCard = document.getElementById('upcomingClassWarningCard');
    const nextClass = getNextClass();

    if (hour >= 13) {
        infoCard.classList.add('hidden'); warnCard.classList.add('hidden');
        document.getElementById('timerContainer').classList.add('hidden'); nextClassGlobal = null; return;
    }

    if (nextClass) {
        infoCard.classList.remove('hidden');
        const cFormatted = formatKopelClass(nextClass.class, nextClass.mapel);
        document.getElementById('nextClassText').innerHTML = `Hari ini Ustadz akan mengajar <b>${nextClass.mapel}</b> di kelas <b>${cFormatted}</b> pada jam <b>${nextClass.masuk}</b> (${nextClass.jp}).`;
        nextClassGlobal = nextClass; document.getElementById('timerContainer').classList.remove('hidden'); updateNextClassTimer();
        ...
        if (diffSeconds > 0 && diffSeconds <= 300) {
            warnCard.classList.remove('hidden'); infoCard.classList.add('hidden');
            const m = Math.floor(diffSeconds / 60); const s = diffSeconds % 60;
            document.getElementById('upcomingTimer').textContent = String(m).padStart(2, '0') + ':' + String(s).padStart(2, '0');
            let isCurrentlyTeaching = JSON.parse(localStorage.getItem('activeTeaching_' + currentUser) || "null") !== null;

            if (isCurrentlyTeaching) {
                warnCard.classList.add('pulse-alarm');
                document.getElementById('upcomingClassWarningText').innerHTML = `<i class="fa-solid fa-bell fa-shake"></i> PERHATIAN!<br>SEGERA PINDAH KE KELAS ${cFormatted} UNTUK MENGAJAR ${nextClass.mapel}!`;
                if (!window.nextClassNotifiedTeaching) { playShortBeep3x(); window.nextClassNotifiedTeaching = true; }
                stopContinuousAlarm(); document.getElementById('muteAlarmBtn').classList.add('hidden');
            } else {
                warnCard.classList.remove('pulse-alarm');
                document.getElementById('upcomingClassWarningText').innerHTML = `<i class="fa-solid fa-triangle-exclamation fa-shake"></i> PERSIAPAN!<br>5 MENIT LAGI MASUK KELAS ${cFormatted} (${nextClass.mapel})!`;
                if (!window.isAlarmMuted && !window.continuousAlarmInterval) {
                    window.continuousAlarmInterval = setInterval(playContinuousBeep, 1500);
                    document.getElementById('muteAlarmBtn').classList.remove('hidden');
                }
            ...
            warnCard.classList.add('hidden'); warnCard.classList.remove('pulse-alarm'); window.nextClassNotifiedTeaching = false;
            if(diffSeconds > 300) window.isAlarmMuted = false;
            stopContinuousAlarm(); infoCard.classList.remove('hidden');
        }
    } else {
        infoCard.classList.add('hidden'); warnCard.classList.add('hidden'); document.getElementById('timerContainer').classList.add('hidden');
        nextClassGlobal = null; stopContinuousAlarm();
    }
}
...
function checkTomorrowSchedule() {
    ...
    const oldTomorrowCard = document.getElementById('tomorrowScheduleCard');
    if (oldTomorrowCard) oldTomorrowCard.classList.add('hidden');
    const now = getCurrentWITA(); const title = document.getElementById('myScheduleTitle');
    if (title) {
        const days = ['ahad', 'senin', 'selasa', 'rabu', 'kamis', 'jumat', 'sabtu'];
        const target = new Date(now);
        if (now.getHours() >= 13) target.setDate(target.getDate() + 1);
        title.textContent = now.getHours() >= 13 ? `Jadwal Saya Esok Hari (${days[target.getDay()].toUpperCase()})` : `Jadwal Saya Hari Ini (${days[target.getDay()].toUpperCase()})`;
    }
}
...
function updateTeachingTimer() {
    ...
    if (diffSeconds > 0) {
        ...
        if (diffSeconds <= 300) { 
            if (!activeTeaching.notified) { playBeep3x(); activeTeaching.notified = true; localStorage.setItem('activeTeaching_' + currentUser, JSON.stringify(activeTeaching)); }
            const warnEl = document.getElementById('activeClassWarning'); warnEl.classList.remove('hidden'); 
            warnEl.textContent = `Waktu mengajar di kelas ini akan segera berakhir (pukul ${activeTeaching.keluar})`;
        } else { document.getElementById('activeClassWarning').classList.add('hidden'); }
    } else {
        document.getElementById('activeClassCountdown').textContent = "00:00"; document.getElementById('activeClassWarning').classList.add('hidden');
    }
}

async function renderMySchedule() {
    if (!currentUser) return;
    const now = getCurrentWITA(); const isAfternoon = now.getHours() >= 13;
    const targetDate = new Date(now);
    if (isAfternoon) targetDate.setDate(targetDate.getDate() + 1);
    const days = ['ahad', 'senin', 'selasa', 'rabu', 'kamis', 'jumat', 'sabtu'];
    const targetDay = days[targetDate.getDay()]; const targetDateStr = getLocalDateString(targetDate);

    let attendedList = [];
    if (!isAfternoon) {
        try {
            const history = await getLocalHistory(currentUser, targetDateStr);
            attendedList = [...history, ...serverDataCache.filter(d => d.teacher === currentUser && d.action === 'masuk' && d.date === targetDateStr)];
        } catch(e) {}
    }

    const blocks = getGroupedSchedule(currentUser, targetDay, isAfternoon ? 0 : (now.getHours() * 60 + now.getMinutes()), targetDate);
    const card = document.getElementById('myScheduleCard'); const tbody = document.getElementById('myScheduleBody'); const title = document.getElementById('myScheduleTitle');
    if (!card || !tbody) return;
    tbody.innerHTML = '';
    
    if (title) title.textContent = isAfternoon ? `Jadwal Saya Esok Hari (${targetDay.toUpperCase()})` : `Jadwal Saya Hari Ini (${targetDay.toUpperCase()})`;
    const oldTomorrowCard = document.getElementById('tomorrowScheduleCard'); if (oldTomorrowCard) oldTomorrowCard.classList.add('hidden');

    if (blocks.length === 0) {
        card.classList.remove('hidden'); card.style.display = 'block';
        tbody.innerHTML = `<tr><td colspan="5" class="text-center text-muted" style="padding:18px;"><i class="fa-solid fa-calendar-xmark" style="font-size:1.4rem;"></i><br>${isAfternoon ? 'Tidak ada jadwal mengajar esok hari.' : 'Tidak ada jadwal mengajar hari ini.'}</td></tr>`;
        return;
    }

    card.classList.remove('hidden'); card.style.display = 'block';
    let countDisplayed = 0; const currentMins = now.getHours() * 60 + now.getMinutes();

    blocks.forEach(sch => {
        const mMins = parseTimeToMins(sch.masuk);
        const isOngoing = !isAfternoon && currentMins >= mMins && currentMins <= sch.kMins;
        const isPassed = !isAfternoon && sch.kMins < currentMins;
        const jpStr = sch.jpStr;
        let hasClockedIn = false;
        ...
        if (!isAfternoon) {
            hasClockedIn = attendedList.some(h => {
                ...
                sch.jps.forEach(j => {
                    if (h.jp) {
                        const hJps = String(h.jp).replace('JP ', '').split('-');
                        const hStart = parseInt(hJps[0], 10); const hEnd = hJps.length > 1 ? parseInt(hJps[1], 10) : hStart;
                        if (j >= hStart && j <= hEnd) overlap = true;
                    }
                });
                if (!overlap) return false;
                ...
                const teacher = TEACHERS.find(t => t.name === currentUser);
                const isGuruTahfizh = teacher && (teacher.subjects || []).includes("Tahfizhul Qur'an");
                if (isGuruTahfizh) {
                    const cleanSchClass = String(sch.class).replace(/[()]/g, ''); const cleanHClass = String(h.class || '').replace(/[()]/g, '');
                    const kopelGroups = [['7A', '7B', '7C'], ['7D', '7E', '8A'], ['8B', '8C', '8D', '8E'], ['9A', '9B', '9C', '9D']];
                    for (const group of kopelGroups) { if (group.some(c => cleanSchClass.includes(c)) && group.some(c => cleanHClass.includes(c))) return true; }
                }
                return false;
            });
        }
        ...
        if (!isAfternoon && isPassed && !hasClockedIn) {
            tr.className = "bg-red-50";
            tr.innerHTML = `<td class="text-danger font-bold text-xs">${jpStr}</td><td class="text-danger text-xs">${sch.masuk}-${sch.keluar}</td><td class="text-danger font-bold text-xs">${sch.class}</td><td class="text-danger text-xs">${sch.mapel}</td><td class="text-xs"><span class="badge bg-red-100 text-red-700 font-bold px-2 py-1">Terlewat</span></td>`;
        } else if (isOngoing) {
            tr.className = "bg-amber-50";
            tr.innerHTML = `<td class="font-bold text-amber-800 text-xs">${jpStr}</td><td class="text-amber-800 text-xs">${sch.masuk}-${sch.keluar}</td><td class="font-bold text-amber-800 text-xs">${sch.class}</td><td class="text-amber-800 text-xs">${sch.mapel}</td><td class="text-xs"><span class="badge bg-amber-100 text-amber-700 font-bold px-2 py-1"><i class="fa-solid fa-spinner fa-spin"></i> Berlangsung</span></td>`;
        } else if (isAfternoon) {
            tr.className = "bg-indigo-50";
            tr.innerHTML = `<td class="font-bold text-indigo-800 text-xs">${jpStr}</td><td class="text-indigo-800 text-xs">${sch.masuk}-${sch.keluar}</td><td class="font-bold text-indigo-800 text-xs">${sch.class}</td><td class="text-indigo-800 text-xs">${sch.mapel}</td><td class="text-xs"><span class="badge" style="background:#e0e7ff;color:#4338ca;font-weight:700;padding:4px 8px;">Esok</span></td>`;
        } else {
            tr.innerHTML = `<td class="font-bold text-xs">${jpStr}</td><td class="text-xs">${sch.masuk}-${sch.keluar}</td><td class="font-bold text-xs">${sch.class}</td><td class="text-xs">${sch.mapel}</td><td class="text-xs"><span class="badge bg-blue-100 text-blue-700 font-bold px-2 py-1">Akan Datang</span></td>`;
        }
        tbody.appendChild(tr);
    });
    ...
    if (countDisplayed === 0) {
        card.classList.remove('hidden'); card.style.display = 'block';
        tbody.innerHTML = `<tr><td colspan="5" class="text-center text-muted" style="padding:18px;">${isAfternoon ? 'Tidak ada jadwal mengajar esok hari.' : 'Semua jadwal hari ini sudah selesai/tercatat.'}</td></tr>`;
    }
}
...
function updateClock() {
    ...
    if (currentUser && window.lastKnownJP !== undefined && window.lastKnownJP !== currentJP) {
        let isCurrentlyTeaching = JSON.parse(localStorage.getItem('activeTeaching_' + currentUser) || "null") !== null;
        if (!isCurrentlyTeaching && currentJP !== 'Di luar JP') playJpChangeAlarm();
    }
    window.lastKnownJP = currentJP;
    updateTeachingTimer(); updateNextClassInfo(); checkTomorrowSchedule();
    if (now.getSeconds() === 0) renderJadwalGrid(); 
    autoResolveLupaAbsen(); 
}
setInterval(updateClock, 1000);
setInterval(() => { if (currentUser) renderMySchedule(); }, 60000);
...
function generateHistoryRowHTML(record) {
    if (record.isMissed) {
        return `<td class="text-danger font-bold">${record.time}</td><td class="font-bold text-danger">${record.class}</td><td class="text-danger">${record.subject}</td><td class="text-danger">❌ Terlewat</td><td class="text-xs text-danger font-bold">${record.keterangan}</td>`;
    }
    const tolMasuk = parseInt(localStorage.getItem('tolMasuk')) || 3;
    let isLate = false;
    if (record.action === 'masuk' && record.jadwal_masuk && record.jadwal_masuk !== '-') {
        let m = parseTimeToMins(record.jadwal_masuk); let wM = parseTimeToMins(record.time);
        if (m > 0 && wM > 0 && (wM - m) > tolMasuk) isLate = true;
    }
    let ketDisplay = record.keterangan || '-'; let editBtn = ''; const todayStr = getLocalDateString(new Date());
    if (isLate && record.date === todayStr) {
        const encodedRecord = encodeURIComponent(JSON.stringify(record));
        editBtn = `<br><button class="btn btn-sm btn-outline mt-1 text-warning" style="padding: 4px 8px; font-size: 0.7rem; width: 100%; border-color: var(--warning);" onclick="editKeterangan('${encodedRecord}')"><i class="fa-solid fa-pen"></i> Edit Alasan</button>`;
    }
...
function addHistoryRow(record) {
    const tbody = document.getElementById('historyBody'); const tr = document.createElement('tr');
    tr.innerHTML = generateHistoryRowHTML(record); tbody.prepend(tr);
    if (tbody.children.length > 50) tbody.removeChild(tbody.lastChild);
    document.getElementById('historyCount').textContent = tbody.children.length;
}
...
function loadHistory() {
    ...
    getLocalHistory(currentUser, getLocalDateString(new Date())).then((localData) => {
        ...
        const serverData = serverDataCache.filter(d => d.teacher === currentUser && d.date === todayStr);
        const mergedMap = new Map();
        serverData.forEach(d => { const key = `${d.class}_${d.subject}_${d.action}_${d.jp}`; mergedMap.set(key, { ...d, timestamp: d.timestamp || d.date + 'T' + d.time }); });
        localData.forEach(d => { const key = `${d.class}_${d.subject}_${d.action}_${d.jp}`; mergedMap.set(key, d); });
        const mergedData = Array.from(mergedMap.values());
        
        const hari = getDayName(); const currentMins = new Date().getHours() * 60 + new Date().getMinutes();
        let blocks = getGroupedSchedule(currentUser, hari, currentMins, new Date());
        let processedKopel = new Set();
        ...
        blocks.forEach(b => {
            ...
            if (currentMins > (mMins + 15)) {
                let jpStr = b.jps.length > 1 ? `JP ${b.jps[0]}-${b.jps[b.jps.length-1]}` : `JP ${b.jps[0]}`;
                let fClass = formatKopelClass(b.class, b.mapel); let fClassClean = fClass.replace(/[()]/g, '');
                let uniqueKey = jpStr + '_' + fClassClean;
                if (processedKopel.has(uniqueKey)) return; processedKopel.add(uniqueKey);
                
                let isScanned = mergedData.some(d => { let dClassClean = (d.class || '').replace(/[()]/g, ''); return d.action === 'masuk' && d.jp === jpStr && dClassClean === fClassClean; });
                if (!isScanned) { mergedData.push({ isMissed: true, time: '-', class: fClassClean, subject: b.mapel, action: 'masuk', keterangan: 'Tidak Scan Masuk', timestamp: todayStr + 'T' + b.masuk + ':00' }); }
            }
        });
        ...
        mergedData.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)).forEach(record => {
            const tr = document.createElement('tr');
            if (record.isMissed) tr.className = "bg-red-50"; 
            tr.innerHTML = generateHistoryRowHTML(record); tbody.appendChild(tr);
        });
        document.getElementById('historyCount').textContent = tbody.children.length; renderMySchedule();
    });
}
...
async function autoResolveLupaAbsen() {
    ...
    if (!activeStr) return;
    let activeData = JSON.parse(activeStr);
    const todayStr = getLocalDateString(new Date()); const now = new Date(); const currentMins = now.getHours() * 60 + now.getMinutes();
    let keluarMins = parseTimeToMins(activeData.keluar); let isPembiasaan = activeData.jp === 'Pembiasaan Pagi';
    let triggerMins = isPembiasaan ? parseTimeToMins('07:29') : (keluarMins + 20);

    if (activeData.date === todayStr && currentMins >= triggerMins) {
        let outH = Math.floor(triggerMins / 60); let outM = triggerMins % 60;
        let autoOutTime = String(outH).padStart(2, '0') + ':' + String(outM).padStart(2, '0') + ':00';
        const recordLupa = { teacher: currentUser, class: activeData.class, subject: activeData.subject, action: 'keluar', date: activeData.date, time: autoOutTime, day: getDayNameID(), jp: activeData.jp || '-', jadwal_masuk: activeData.masuk || '-', jadwal_keluar: activeData.keluar, timestamp: now.toISOString(), keterangan: 'lupa scan keluar', synced: false };
        
        await saveAttendanceLocal(recordLupa);
        if (navigator.onLine) { sendToGoogleSheet(recordLupa).then(res => { if(res.success) processSyncedItem(recordLupa); }); }
        localStorage.removeItem('activeTeaching_' + currentUser);
        const timerCard = document.getElementById('activeClassTimerCard'); if(timerCard) timerCard.classList.add('hidden');
        updateScanModeUI(); showToast('Sistem mencatat otomatis absen keluar (' + (isPembiasaan ? 'Batas Pembiasaan Pagi 08:29' : 'Lupa > 20 Menit') + ').', 'warning');
    }
}
...
async function verifySheetStatus() {
    ...
    let activeStr = localStorage.getItem('activeTeaching_' + currentUser);
    if (!activeStr) return; 
    await refreshServerCache(); const todayStr = getLocalDateString(new Date());
    let activeData; try { activeData = JSON.parse(activeStr); } catch (e) { return; }

    const relevantToday = serverDataCache.filter(d => d.date === todayStr && d.teacher === currentUser && d.class === activeData.class && d.jp === activeData.jp );
    if (relevantToday.length > 0) {
        const lastActivity = relevantToday.reduce((a, b) => (new Date(a.timestamp) > new Date(b.timestamp) ? a : b));
        if (lastActivity.action === 'keluar') {
            localStorage.removeItem('activeTeaching_' + currentUser);
            const timerCard = document.getElementById('activeClassTimerCard'); if (timerCard) timerCard.classList.add('hidden');
            currentMode = 'masuk'; updateScanModeUI();
        }
    }
}
...
function checkMidnightReset() {
    const now = getCurrentWITA();
    if (now.getHours() === 0 && now.getMinutes() === 0 && now.getSeconds() === 0) updateNextClassTimer();
}
setInterval(checkMidnightReset, 60000);
...
function updateNextClassTimer() {
    if (!currentUser) return;
    const now = getCurrentWITA(); const hour = now.getHours();
    if (hour >= 13) { document.getElementById('timerContainer').classList.add('hidden'); return; }
    let nextClass = nextClassGlobal;
    if (!nextClass) { nextClass = getNextClass(); if (nextClass) nextClassGlobal = nextClass; }
    if (!nextClass) { document.getElementById('timerContainer').classList.add('hidden'); return; }

    const masukMins = parseTimeToMins(nextClass.masuk); const totalMins = now.getHours() * 60 + now.getMinutes();
    const diffMins = masukMins - totalMins;
    if (diffMins <= 0) { document.getElementById('timerContainer').classList.add('hidden'); return; }
    const hours = Math.floor(diffMins / 60); const minutes = diffMins % 60; const seconds = 60 - now.getSeconds();
    document.getElementById('nextClassTimer').textContent = `${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
    document.getElementById('timerContainer').classList.remove('hidden');
}

// ==========================================
// BLOK 5: FITUR NILAI, IZIN/KASUS, WALI KELAS
// ==========================================
let tempNilaiList = [];
let currentGuruScheduleData = null;
let tempSantriIzinList = [];
...
function getGuruScheduleData(guruName) {
    let scheduleData = {}; let allMapel = new Set();
    let teacherInfo = TEACHERS.find(t => t.name === guruName); let teacherSubjects = teacherInfo ? teacherInfo.subjects : [];
    ...
    days.forEach(hari => {
        if (jadwalPelajaran[hari]) {
            Object.keys(jadwalPelajaran[hari]).forEach(jp => {
                Object.keys(jadwalPelajaran[hari][jp]).forEach(kelas => {
                    let jadwalRaw = jadwalPelajaran[hari][jp][kelas]; let mapelFound = null;
                    if (jadwalRaw.includes(guruName)) { mapelFound = jadwalRaw.split('(')[0].trim(); } 
                    else if (jadwalRaw.includes('SBDP / Informatika')) {
                        if (isInformatika) mapelFound = 'Informatika'; if (isSBDP) mapelFound = 'SBDP';
                    }
                    if (mapelFound) {
                        if (!scheduleData[kelas]) scheduleData[kelas] = new Set();
                        scheduleData[kelas].add(mapelFound); allMapel.add(mapelFound);
                    }
                });
            });
        }
    ...
    if (isGuruTahfizh) { CLASSES.forEach(c => { if (!scheduleData[c]) scheduleData[c] = new Set(); scheduleData[c].add("Tahfizhul Qur'an"); allMapel.add("Tahfizhul Qur'an"); }); }
    for (let kls in scheduleData) { scheduleData[kls] = Array.from(scheduleData[kls]); }
    return { classesMapping: scheduleData, uniqueMapel: Array.from(allMapel) };
}

function initNilaiTab() {
    const classSelect = document.getElementById('nilaiKelasSelect'); const mapelSelect = document.getElementById('nilaiMapelInput'); const filterClassSelect = document.getElementById('filterRekapNilaiKelas');
    document.getElementById('nilaiTanggalInput').value = getLocalDateString(new Date());
    currentGuruScheduleData = getGuruScheduleData(currentUser);

    classSelect.innerHTML = '<option value="">-- Pilih Kelas --</option>'; filterClassSelect.innerHTML = '<option value="ALL">-- Semua Kelas --</option>'; mapelSelect.innerHTML = '<option value="">-- Pilih Mapel --</option>';
    const diajarKelas = Object.keys(currentGuruScheduleData.classesMapping).sort();
    
    if (diajarKelas.length > 0) { diajarKelas.forEach(cls => { classSelect.innerHTML += `<option value="${cls}">${cls}</option>`; filterClassSelect.innerHTML += `<option value="${cls}">${cls}</option>`; }); } 
    else { CLASSES.forEach(cls => { classSelect.innerHTML += `<option value="${cls}">${cls}</option>`; filterClassSelect.innerHTML += `<option value="${cls}">${cls}</option>`; }); }

    if (currentGuruScheduleData.uniqueMapel.length > 0) {
        currentGuruScheduleData.uniqueMapel.forEach(mpl => { mapelSelect.innerHTML += `<option value="${mpl}">${mpl}</option>`; });
        if (currentGuruScheduleData.uniqueMapel.length === 1) mapelSelect.value = currentGuruScheduleData.uniqueMapel[0];
    }

    let activeTeaching = JSON.parse(localStorage.getItem('activeTeaching_' + currentUser) || "null");
    if (activeTeaching && !activeTeaching.keluarScanned) {
        document.getElementById('badgeStatusKelasNilai').innerHTML = '<i class="fa-solid fa-link"></i> Terhubung ke Kelas Aktif';
        document.getElementById('badgeStatusKelasNilai').style.background = '#ecfdf5'; document.getElementById('badgeStatusKelasNilai').style.color = '#059669';
        let rawClass = activeTeaching.rawClass || activeTeaching.class.split(',')[0].replace(/[()]/g, '');
        if([...classSelect.options].some(opt => opt.value === rawClass)) { classSelect.value = rawClass; classSelect.dispatchEvent(new Event('change')); }
        setTimeout(() => { if([...mapelSelect.options].some(opt => opt.value === activeTeaching.subject)) { mapelSelect.value = activeTeaching.subject; } else { mapelSelect.innerHTML += `<option value="${activeTeaching.subject}">${activeTeaching.subject}</option>`; mapelSelect.value = activeTeaching.subject; } }, 50);
    } else {
        document.getElementById('badgeStatusKelasNilai').innerHTML = 'Mode: Manual / Luar Jam';
        document.getElementById('badgeStatusKelasNilai').style.background = '#eff6ff'; document.getElementById('badgeStatusKelasNilai').style.color = '#2563eb';
    }
}

function renderTempNilaiTable() {
    const tbody = document.getElementById('tempNilaiBody'); const kirimBtn = document.getElementById('kirimNilaiBtn'); tbody.innerHTML = '';
    document.getElementById('countTempNilai').textContent = `${tempNilaiList.length} Santri`;
    if (tempNilaiList.length === 0) { tbody.innerHTML = '<tr><td colspan="3" class="text-center text-muted">Belum ada nilai ditambahkan</td></tr>'; kirimBtn.classList.add('hidden'); return; }
    kirimBtn.classList.remove('hidden');
    tempNilaiList.forEach((item, index) => {
        let tr = document.createElement('tr');
        tr.innerHTML = `<td class="font-bold text-xs">${item.nama_santri}</td><td class="text-center font-black text-blue-700">${item.nilai}</td><td class="text-center"><button class="btn btn-sm btn-danger" style="padding:4px 8px; font-size:0.75rem;" onclick="removeTempNilai(${index})"><i class="fa-solid fa-trash"></i></button></td>`;
        tbody.appendChild(tr);
    });
}
window.removeTempNilai = function(index) { tempNilaiList.splice(index, 1); renderTempNilaiTable(); };
...
async function loadRekapNilai() {
    ...
    tbody.innerHTML = '<tr><td colspan="3" class="text-center py-6 text-muted"><i class="fa-solid fa-spinner fa-spin mr-2"></i> Mengambil rekap nilai...</td></tr>';
    try {
        const response = await fetch(GAS_URL + '?action=getNilai&t=' + new Date().getTime()); const res = await response.json();
        if (!res.success) throw new Error(res.error);
        serverDataNilai = res.data; renderRekapNilaiTable();
    } catch (e) { tbody.innerHTML = `<tr><td colspan="3" class="text-center text-danger">Gagal: ${e.message}</td></tr>`; }
}

function renderRekapNilaiTable() {
    const tbody = document.getElementById('rekapNilaiBody'); const filterKelas = document.getElementById('filterRekapNilaiKelas').value; const filterJenis = document.getElementById('filterRekapNilaiJenis').value;
    let filtered = serverDataNilai.filter(d => d.guru === currentUser);
    if (filterKelas !== 'ALL') filtered = filtered.filter(d => d.kelas === filterKelas);
    if (filterJenis !== 'ALL') filtered = filtered.filter(d => d.jenis_nilai === filterJenis);
    ...
    tbody.innerHTML = '';
    if (filtered.length === 0) { tbody.innerHTML = '<tr><td colspan="3" class="text-center text-muted py-4">Belum ada data nilai.</td></tr>'; return; }
    filtered.forEach(d => {
        let tr = document.createElement('tr');
        tr.innerHTML = `<td class="text-xs"><span class="font-bold text-blue-700">${d.nama_penilaian}</span><br><span class="text-muted" style="font-size: 0.7rem;">${d.jenis_nilai} | Kls ${d.kelas}</span></td><td class="font-bold text-xs">${d.nama_santri}</td><td class="text-center font-black">${d.nilai}</td>`;
        tbody.appendChild(tr);
    });
}

function renderSantriIzinTable() {
    const tbody = document.getElementById('santriIzinBody'); const kirimBtn = document.getElementById('kirimSantriIzinBtn'); tbody.innerHTML = '';
    if (tempSantriIzinList.length === 0) { tbody.innerHTML = '<tr><td colspan="3" class="text-center text-muted">Belum ada santri ditambahkan</td></tr>'; kirimBtn.classList.add('hidden'); return; }
    kirimBtn.classList.remove('hidden');
    tempSantriIzinList.forEach((item, index) => {
        let tr = document.createElement('tr');
        tr.innerHTML = `<td class="font-bold">${item.nama}</td><td><span class="badge bg-amber-100 text-amber-800">${item.jenis_izin}</span></td><td><button class="btn btn-sm btn-danger" style="padding:4px 8px; font-size:0.75rem;" onclick="removeTempSantriIzin(${index})"><i class="fa-solid fa-trash"></i></button></td>`;
        tbody.appendChild(tr);
    });
}
window.removeTempSantriIzin = function(index) { tempSantriIzinList.splice(index, 1); renderSantriIzinTable(); };
...
function renderSantriKasusTable() {
    const tbody = document.getElementById('santriKasusBody'); const kirimBtn = document.getElementById('kirimSantriKasusBtn'); tbody.innerHTML = '';
    if (tempSantriKasusList.length === 0) { tbody.innerHTML = '<tr><td colspan="3" class="text-center text-muted">Belum ada kasus dicatat</td></tr>'; kirimBtn.classList.add('hidden'); return; }
    kirimBtn.classList.remove('hidden');
    tempSantriKasusList.forEach((item, index) => {
        let tr = document.createElement('tr');
        tr.innerHTML = `<td class="font-bold">${item.nama}</td><td><span class="badge bg-red-100 text-red-700">${item.jenis_kasus}</span></td><td><button class="btn btn-sm btn-danger" style="padding:4px 8px; font-size:0.75rem;" onclick="removeTempSantriKasus(${index})"><i class="fa-solid fa-trash"></i></button></td>`;
        tbody.appendChild(tr);
    });
}
window.removeTempSantriKasus = function(index) { tempSantriKasusList.splice(index, 1); renderSantriKasusTable(); };
...
async function renderPembiasaanWali() {
    ...
    try {
        const res = await (await fetch(GAS_URL + '?action=getAbsensi')).json();
        const period = document.getElementById('filterPembiasaanPeriodeWali').value; const now = new Date();
        let filtered = res.data.filter(d => d.teacher === currentUser && d.jp === 'Pembiasaan Pagi');
        ...
        tbody.innerHTML = '';
        Object.keys(mapPagi).sort().reverse().forEach(tgl => {
            let r = mapPagi[tgl]; let telat = false; let mMins = parseTimeToMins('07:15'); let wMins = parseTimeToMins(r.in);
            if (wMins > (mMins + 3)) telat = true;
            let statusBadge = telat ? `<span class="badge bg-amber-100 text-amber-700 font-bold px-2 py-1 text-xs">Telat</span>` : `<span class="badge bg-emerald-100 text-emerald-700 font-bold px-2 py-1 text-xs">Tepat</span>`;
            tbody.innerHTML += `<tr><td class="font-bold text-xs">${tgl}</td><td>${r.in}</td><td>${r.out}</td><td>${statusBadge}</td></tr>`;
        });
...
function cekPembiasaanPagi(namaGuru) {
    ...
    if (WALI_KELAS.includes(namaGuru)) {
        let now = new Date();
        if (now.getDay() >= 1 && now.getDay() <= 6) {
            let jam = now.getHours(); let menit = now.getMinutes(); let waktuDesimal = jam + (menit / 60);
            if (waktuDesimal >= 7.25 && waktuDesimal <= 7.30) { return true; } 
            else if (waktuDesimal > 7.30 && waktuDesimal <= 7.50) { return "Telat Pembiasaan Pagi"; }
        }
    }
    return false;
}
...
// ==========================================
// BLOK 6: DASHBOARD ADMIN, MASTER & REKAP
// ==========================================
async function loadRekap() {
    if (!currentUser) return;
    const tbody = document.getElementById('rekapBody'); 
    ...
    try {
        const response = await fetch(GAS_URL + '?action=getAbsensi'); const res = await response.json();
        if (!res.success) throw new Error(res.error);
        ...
        let filtered = res.data.filter(d => {
            ...
            if (rekapFilter === 'day') return d.date === today;
            let filterDate = new Date(); filterDate.setHours(0,0,0,0);
            if (rekapFilter === 'week') { filterDate.setDate(filterDate.getDate() - filterDate.getDay() + 1); return d.date >= filterDate.toISOString().split('T')[0]; }
            if (rekapFilter === 'month') { filterDate.setDate(1); return d.date >= filterDate.toISOString().split('T')[0]; }
            return true; 
        });
        ...
        filtered.forEach(d => {
            if (!d.date || !d.class || !d.jp) return;
            const key = d.date + '_' + d.class + '_' + d.jp;
            if (!grouped[key]) grouped[key] = { date: d.date, class: d.class, subject: d.subject, jp: d.jp, jadwal_masuk: d.jadwal_masuk || '-', jadwal_keluar: d.jadwal_keluar || '-', waktu_masuk: '-', waktu_keluar: '-', ket: d.keterangan || '-', isMissed: false };
            if (d.action === 'masuk') { grouped[key].waktu_masuk = d.time; grouped[key].jadwal_masuk = d.jadwal_masuk || grouped[key].jadwal_masuk; if(d.keterangan) grouped[key].ket = d.keterangan; }
            if (d.action === 'keluar') { grouped[key].waktu_keluar = d.time; grouped[key].jadwal_keluar = d.jadwal_keluar || grouped[key].jadwal_keluar; if(d.keterangan && grouped[key].ket === '-') grouped[key].ket = d.keterangan; }
        });

        let startDate = new Date(); startDate.setHours(0,0,0,0);
        if (rekapFilter === 'week') startDate.setDate(startDate.getDate() - startDate.getDay() + 1);
        else if (rekapFilter === 'month') startDate.setDate(1);
        else if (rekapFilter === 'all') startDate = new Date(startDate.getFullYear(), 6, 1); 
        
        let tempDate = new Date(startDate); const todayStrLocal = getLocalDateString(new Date()); const nowMins = new Date().getHours() * 60 + new Date().getMinutes();
        while (getLocalDateString(tempDate) <= todayStrLocal) {
            if (tempDate.getDay() !== 0 && !isLiburSekolah(tempDate)) {
                let dStr = getLocalDateString(tempDate); let dHari = ['ahad', 'senin', 'selasa', 'rabu', 'kamis', 'jumat', 'sabtu'][tempDate.getDay()];
                let blocks = getGroupedSchedule(currentUser, dHari, 0, tempDate);
                let processedRekapKopel = new Set();
                blocks.forEach(b => {
                    let isPassed = true;
                    if (dStr === todayStrLocal) { let m = parseTimeToMins(b.masuk); if (nowMins < (m + 15)) isPassed = false; }
                    if (isPassed) {
                        let jpStr = b.jps.length > 1 ? `JP ${b.jps[0]}-${b.jps[b.jps.length-1]}` : `JP ${b.jps[0]}`;
                        let fClass = formatKopelClass(b.class, b.mapel); let fClassClean = fClass.replace(/[()]/g, ''); 
                        let kopelKey = dStr + '_' + fClassClean + '_' + jpStr;
                        if (processedRekapKopel.has(kopelKey)) return; processedRekapKopel.add(kopelKey);
                        
                        let isScanned = Object.values(grouped).some(g => { return g.date === dStr && g.jp === jpStr && String(g.class).replace(/[()]/g, '') === fClassClean; });
                        if (!isScanned) { grouped[kopelKey] = { date: dStr, class: fClassClean, subject: b.mapel, jp: jpStr, jadwal_masuk: b.masuk, jadwal_keluar: b.keluar, waktu_masuk: '-', waktu_keluar: '-', ket: 'Tdk Scan (Alpa)', isMissed: true }; }
                    }
                });
            }
            tempDate.setDate(tempDate.getDate() + 1);
        ...
        tbody.innerHTML = '';
        if (finalData.length === 0) {
            document.getElementById('rekapEmpty').classList.remove('hidden'); document.getElementById('downloadArea').classList.add('hidden');
            document.getElementById('rekapSummary').querySelectorAll('.num').forEach(el => el.textContent = '0'); 
            return;
        }
        document.getElementById('rekapEmpty').classList.add('hidden'); document.getElementById('downloadArea').classList.remove('hidden');

        finalData.forEach(r => {
            let isLate = false, isEarly = false; let telatMenit = 0, cepatMenit = 0;
            let jM = parseTimeToMins(r.jadwal_masuk); let wM = parseTimeToMins(r.waktu_masuk);
            if (jM > 0 && wM > 0) { let diff = wM - jM; if (diff > tolMasuk) { telatMenit = diff - tolMasuk; lateCount += telatMenit; isLate = true; } }
            let jK = parseTimeToMins(r.jadwal_keluar); let wK = parseTimeToMins(r.waktu_keluar);
            if (jK > 0 && wK > 0) { let diff = jK - wK; if (diff > tolKeluar) { cepatMenit = diff - tolKeluar; earlyCount += cepatMenit; isEarly = true; } }

            let statusHTML = ''; const tr = document.createElement('tr');
            if (r.isMissed) { statusHTML = '<span class="badge bg-red-100 text-red-700 px-2 py-1 rounded text-xs font-bold">Alpa / Terlewat</span>'; missedCount++; tr.className = "bg-red-50"; } 
            else if (r.waktu_masuk === '-' || r.waktu_keluar === '-') {
                statusHTML = '<span class="badge bg-gray-100 text-gray-700 px-2 py-1 rounded text-xs font-bold">Incomplete</span>';
                if (isLate) statusHTML += `<br><span class="badge bg-amber-100 text-amber-700 px-2 py-1 mt-1 rounded text-xs font-bold">Telat ${telatMenit}m</span>`;
                if (isEarly) statusHTML += `<br><span class="badge bg-amber-100 text-amber-700 px-2 py-1 mt-1 rounded text-xs font-bold">Cepat ${cepatMenit}m</span>`;
            ...
            tr.innerHTML = `<td>${r.date}</td><td class="font-bold">${r.class}</td><td>${r.subject}</td><td>${r.waktu_masuk}</td><td>${r.waktu_keluar}</td><td>${statusHTML}</td><td class="text-xs ${r.isMissed ? 'text-danger font-bold' : ''}">${r.ket}</td>`;
            tbody.appendChild(tr);
        });
        document.getElementById('statTotal').textContent = finalData.length; document.getElementById('statLate').textContent = lateCount; 
        document.getElementById('statEarly').textContent = earlyCount; document.getElementById('statMissed').textContent = missedCount; 
    } catch (e) { tbody.innerHTML = ''; document.getElementById('rekapEmpty').classList.remove('hidden'); document.getElementById('rekapEmpty').textContent = 'Koneksi gagal saat memuat data server'; }
}
...
function buildAdminMissedClasses(allData) {
    const tbody = document.getElementById('missedClassesBody'); const todayStr = getLocalDateString(new Date()); 
    const serverDataToday = allData.filter(d => d.date === todayStr && d.action === 'masuk');
    const hari = getDayName(); const now = new Date(); const currentMins = now.getHours() * 60 + now.getMinutes();
    let missedList = [];
        ...
        blocks.forEach(sch => {
            ...
            if (currentMins > batasPengecekan) {
                let hasClockedIn = serverDataToday.some(h => {
                    ...
                    sch.jps.forEach(j => {
                        if (h.jp) {
                            let hJps = h.jp.replace('JP ', '').split('-'); let hStart = parseInt(hJps[0]); let hEnd = hJps.length > 1 ? parseInt(hJps[1]) : hStart;
                            if (j >= hStart && j <= hEnd) overlap = true; 
                        }
                    });
                    if (!overlap) return false;
                    ...
                    if (schArr.some(c => hArr.includes(c))) { classMatch = true; }

                    let mapelStr = (sch.mapel || "").toLowerCase(); let hMapelStr = (h.subject || "").toLowerCase();
                    if (!classMatch && mapelStr.includes('tahfiz') && hMapelStr.includes('tahfiz')) {
                        const kopelTahfizh = [['7a', '7b', '7c'], ['7d', '7e', '8a'], ['8b', '8c', '8d', '8e'], ['9a', '9b', '9c', '9d']];
                        for (let group of kopelTahfizh) { if (group.some(c => schArr.includes(c)) && group.some(c => hArr.includes(c))) { classMatch = true; break; } }
                    }
                    if (!classMatch && mapelStr.includes('pjok') && hMapelStr.includes('pjok') && t.name === 'M. Tahir, M. Pd.') {
                        const kopelPJOK = [['9a', '9b'], ['9c', '9d'], ['7d', '7e'], ['7a', '7b'], ['8a', '8b'], ['7c', '8e']];
                        for (let group of kopelPJOK) { if (group.some(c => schArr.includes(c)) && group.some(c => hArr.includes(c))) { classMatch = true; break; } }
                    }
                    return classMatch;
                });
                ...
                if (!hasClockedIn) {
                    let jpStr = sch.jps.length > 1 ? `JP ${sch.jps[0]}-${sch.jps[sch.jps.length-1]}` : `JP ${sch.jps[0]}`;
                    missedList.push({ teacher: t.name, class: sch.class, mapel: sch.mapel, jp: jpStr, waktu: `${sch.masuk} - ${sch.keluar}` });
                }
            }
        });
    });
    ...
    tbody.innerHTML = '';
    if (missedList.length === 0) { tbody.innerHTML = '<tr><td colspan="5" class="text-center py-4 text-success font-bold"><i class="fa-solid fa-check-double"></i> Alhamdulillah, seluruh jadwal kelas yang sedang/telah lewat berhasil diisi (Tidak ada yang absen).</td></tr>'; return; }
    missedList.forEach(m => {
        const tr = document.createElement('tr');
        tr.innerHTML = `<td class="font-bold text-danger text-sm">${m.teacher}</td><td class="font-bold text-sm">${m.class}</td><td class="text-sm">${m.mapel}</td><td class="font-bold text-danger text-sm">${m.jp}</td><td class="text-sm">${m.waktu}</td>`;
        tbody.appendChild(tr);
...
function getOngoingAndNextJP() {
    const now = new Date(); const totalMins = now.getHours() * 60 + now.getMinutes(); const day = getDayName();
    const jadwal = (day === 'jumat') ? JADWAL_WAKTU.jumat : JADWAL_WAKTU.reguler;
    let ongoing = null, next = null; let jps = Object.keys(jadwal).sort();
    for (let i = 0; i < jps.length; i++) {
        let m = parseTimeToMins(jadwal[jps[i]].masuk); let k = parseTimeToMins(jadwal[jps[i]].keluar);
        if (totalMins >= m && totalMins <= k) { ongoing = jps[i]; if (i + 1 < jps.length) next = jps[i+1]; break; } 
        else if (totalMins < m) { if (!next) next = jps[i]; break; }
    }
    return { ongoing, next };
}

function renderLiveMonitor() {
    const tbody = document.getElementById('liveMonitorBody'); const info = document.getElementById('liveMonitorJpInfo');
    if (currentServerDataCacheAdmin.length === 0) { tbody.innerHTML = '<tr><td colspan="4" class="text-center">Belum ada data dari server...</td></tr>'; return; }
    
    const todayStr = getLocalDateString(getCurrentWITA()); const serverDataToday = currentServerDataCacheAdmin.filter(d => d.date === todayStr && d.action === 'masuk');
    const day = getDayName(); const jpInfo = getOngoingAndNextJP(); const targetJp = liveMonitorMode === 'ongoing' ? jpInfo.ongoing : jpInfo.next;
    ...
    const jadwal = (day === 'jumat') ? JADWAL_WAKTU.jumat : JADWAL_WAKTU.reguler;
    info.innerHTML = `<span class="badge bg-indigo-100 text-indigo-800 px-3 py-1 rounded-full text-xs">${targetJp} (${jadwal[targetJp].masuk} - ${jadwal[targetJp].keluar})</span>`;
    let rowsHTML = ''; let renderedClasses = new Set(); 

    CLASSES.forEach(cls => {
        let teachers = []; let mapel = '-';
        TEACHERS.forEach(t => { let m = getMapelForScan(cls, t.name, day, targetJp); if (m) { teachers.push(t.name); mapel = m; } });

        if (teachers.length > 0) {
            let formattedClass = formatKopelClass(cls, mapel);
            if (renderedClasses.has(formattedClass)) return; renderedClasses.add(formattedClass);
            ...
            const isOverlap = (recordJp, checkJp) => {
                if (!recordJp || !checkJp) return false;
                let rJps = recordJp.replace('JP ', '').split('-'); let cJps = checkJp.replace('JP ', '').split('-');
                return (parseInt(rJps[0]) <= (cJps.length > 1 ? parseInt(cJps[1]) : parseInt(cJps[0])) && (rJps.length > 1 ? parseInt(rJps[1]) : parseInt(rJps[0])) >= parseInt(cJps[0]));
            };

            let hasClockedIn = serverDataToday.some(d => {
                if (!isOverlap(d.jp, targetJp)) return false;
                let kopelArray = formattedClass.replace(/[()]/g, '').toLowerCase().split(',').map(c => c.trim());
                let serverClass = (d.class || "").replace(/[()]/g, '').toLowerCase().split(',').map(c => c.trim());
                let classMatch = false;
                ...
                let mapelStr = (mapel || "").toLowerCase(); let hMapelStr = (d.subject || "").toLowerCase();

                if (!classMatch && mapelStr.includes('tahfiz') && hMapelStr.includes('tahfiz')) {
                    const kopelTahfizh = [ ['7a', '7b', '7c'], ['7d', '7e', '8a'], ['8b', '8c', '8d', '8e'], ['9a', '9b', '9c', '9d'] ];
                    for (let group of kopelTahfizh) {
                        let groupMatchSchedule = group.some(c => kopelArray.includes(c)); let groupMatchServer = group.some(c => serverClass.includes(c));
                        if (groupMatchSchedule && groupMatchServer) { classMatch = true; break; }
                    }
                }
                
                if (!classMatch && mapelStr.includes('pjok') && hMapelStr.includes('pjok') && teachers.includes('M. Tahir, M. Pd.')) {
                    const kopelPJOK = [ ['9a', '9b'], ['9c', '9d'], ['7d', '7e'], ['7a', '7b'], ['8a', '8b'], ['7c', '8e'] ];
                    for (let group of kopelPJOK) { if (group.some(c => kopelArray.includes(c)) && group.some(c => serverClass.includes(c))) { classMatch = true; break; } }
                }
            ...
            rowsHTML += `<tr><td class="font-bold text-xs">${formattedClass}</td><td class="text-xs">${mapel}</td><td class="text-xs">${teacherDisplay}</td><td class="text-center">${statusBadge}</td></tr>`;
        }
    });
    tbody.innerHTML = rowsHTML || '<tr><td colspan="4" class="text-center text-muted">Tidak ada jadwal pada JP ini</td></tr>';
}
...
async function loadAdminData() {
    if (!isAdmin) return;
    const tbody = document.getElementById('adminBody'); const missedTbody = document.getElementById('missedClassesBody');
    tbody.innerHTML = '<tr><td colspan="9" class="text-center py-6 font-bold text-gray-500"><i class="fa-solid fa-spinner fa-spin mr-2"></i> Memuat data dari Google Sheets...</td></tr>';
    missedTbody.innerHTML = '<tr><td colspan="5" class="text-center py-4 text-muted"><i class="fa-solid fa-spinner fa-spin"></i> Mengecek rekap...</td></tr>';
    ...
    try {
        const res = await (await fetch(GAS_URL + '?action=getAbsensi')).json();
        if (!res.success) throw new Error(res.error);
        const tolMasuk = parseInt(localStorage.getItem('tolMasuk')) || 3; const tolKeluar = parseInt(localStorage.getItem('tolKeluar')) || 3;
        
        res.data.forEach(d => {
            if (d.date && typeof d.date === 'string' && d.date.includes('T')) {
                let dt = new Date(d.date); if (!isNaN(dt.getTime())) d.date = dt.getFullYear() + '-' + String(dt.getMonth()+1).padStart(2, '0') + '-' + String(dt.getDate()).padStart(2, '0');
            }
            ['time', 'jadwal_masuk', 'jadwal_keluar'].forEach(k => {
                if (d[k] && typeof d[k] === 'string') {
                    if (d[k].includes('T')) { let dt = new Date(d[k]); if (!isNaN(dt.getTime())) d[k] = dt.getHours().toString().padStart(2,'0') + ':' + dt.getMinutes().toString().padStart(2,'0'); } 
                    else { d[k] = d[k].replace(/\./g, ':'); }
                }
            });
        });
        
        currentServerDataCacheAdmin = res.data; renderLiveMonitor(); buildAdminMissedClasses(res.data);
        
        const filterTeacher = document.getElementById('adminTeacherFilter').value; const filterPeriod = document.getElementById('adminPeriodFilter').value; const todayStr = new Date().toISOString().split('T')[0];
        let filtered = res.data.filter(d => {
            if (filterTeacher !== 'all' && d.teacher !== filterTeacher) return false;
            if (filterPeriod === 'day') return d.date === todayStr;
            if (filterPeriod === 'week') return d.date >= new Date(new Date().setDate(new Date().getDate() - new Date().getDay() + 1)).toISOString().split('T')[0];
        ...
        filtered.forEach(d => {
            ...
            if (d.action === 'masuk') { grouped[key].waktu_masuk = d.time; grouped[key].jadwal_masuk = d.jadwal_masuk || grouped[key].jadwal_masuk; }
            if (d.action === 'keluar') { 
                grouped[key].waktu_keluar = d.time; grouped[key].jadwal_keluar = d.jadwal_keluar || grouped[key].jadwal_keluar; 
                if (d.keterangan === 'Lupa scan keluar pada riwayat kelas sebelumnya') grouped[key].isLupa = true;
            }
        });

        let tLate = 0, tEarly = 0, tLupa = 0, visibleRecords = 0; tbody.innerHTML = '';
        const finalData = Object.values(grouped).sort((a,b) => b.date.localeCompare(a.date) || a.teacher.localeCompare(b.teacher));
        
        finalData.forEach(g => {
            let telatMenit = 0, cepatMenit = 0;
            let jM = parseTimeToMins(g.jadwal_masuk), wM = parseTimeToMins(g.waktu_masuk); if (jM > 0 && wM > 0) { let diff = wM - jM; if (diff > tolMasuk) telatMenit = diff - tolMasuk; }
            let jK = parseTimeToMins(g.jadwal_keluar), wK = parseTimeToMins(g.waktu_keluar); if (jK > 0 && wK > 0) { let diff = jK - wK; if (diff > tolKeluar) cepatMenit = diff - tolKeluar; }
            
            if (adminViewMode === 'late' && telatMenit <= 0) return; if (adminViewMode === 'early' && cepatMenit <= 0) return; if (adminViewMode === 'lupa' && !g.isLupa) return;
            
            if (g.isLupa) tLupa++; tLate += telatMenit; tEarly += cepatMenit; visibleRecords++;
            const badgeLupa = g.isLupa ? `<br><span class="badge" style="background:#f3e8ff; color:#9333ea; font-size:0.7rem;">Lupa Scan Keluar</span>` : '';
            const tr = document.createElement('tr');
            tr.innerHTML = `<td>${g.date}</td><td class="font-bold">${g.teacher}</td><td>${g.subject}</td><td>${g.class}</td><td class="font-bold">${g.jp}</td><td>${g.waktu_masuk} <br><span style="font-size:0.7rem;color:gray">Jdw: ${g.jadwal_masuk}</span></td><td>${g.waktu_keluar} <br><span style="font-size:0.7rem;color:gray">Jdw: ${g.jadwal_keluar}</span>${badgeLupa}</td><td class="${telatMenit>0?'text-danger font-bold':''}">${telatMenit}</td><td class="${cepatMenit>0?'text-warning font-bold':''}">${cepatMenit}</td>`;
            tbody.appendChild(tr);
        });
        
        if (visibleRecords === 0) { tbody.innerHTML = '<tr><td colspan="9" class="text-center py-4">Tidak ada data yang sesuai filter.</td></tr>'; }
        document.getElementById('adminStatTotal').textContent = visibleRecords; document.getElementById('adminStatLate').textContent = tLate + ' Mnt'; 
        document.getElementById('adminStatEarly').textContent = tEarly + ' Mnt'; document.getElementById('adminStatLupa').textContent = tLupa + ' Kali';
    } catch (e) { tbody.innerHTML = `<tr><td colspan="9" class="text-center text-danger">Gagal: ${e.message}</td></tr>`; }
}

async function loadAgusData() {
    if (!isAgus) return;
    const tbody = document.getElementById('agusBody'); const period = document.getElementById('agusFilterPeriod').value;
    const tolMasuk = parseInt(localStorage.getItem('tolMasuk')) || 3; const tolKeluar = parseInt(localStorage.getItem('tolKeluar')) || 3;
    const tableHead = document.querySelector('#agusBody').closest('table').querySelector('thead tr');
    tableHead.innerHTML = '<th>Nama Guru</th><th>Mapel</th><th>Total Sesi</th><th>Telat (Mnt)</th><th>Cepat (Mnt)</th><th>Lupa Out</th>';
    tbody.innerHTML = '<tr><td colspan="6" class="text-center py-6 font-bold text-gray-500"><i class="fa-solid fa-spinner fa-spin mr-2"></i> Mengambil data dari Google Sheet...</td></tr>';
    ...
    try {
        ...
        filtered.forEach(d => {
            if (!d.date || !d.teacher || !d.jp) return;
            const key = d.date + '_' + d.teacher + '_' + d.class + '_' + d.jp;
            if (!grouped[key]) grouped[key] = { date: d.date, teacher: d.teacher, mapel: d.subject || '-', jadwal_masuk: d.jadwal_masuk || '-', jadwal_keluar: d.jadwal_keluar || '-', waktu_masuk: '-', waktu_keluar: '-', action_masuk: false, action_keluar: false, isLupa: false };
            if (d.action === 'masuk') { grouped[key].waktu_masuk = d.time; grouped[key].jadwal_masuk = d.jadwal_masuk || grouped[key].jadwal_masuk; grouped[key].action_masuk = true; }
            if (d.action === 'keluar') { grouped[key].waktu_keluar = d.time; grouped[key].jadwal_keluar = d.jadwal_keluar || grouped[key].jadwal_keluar; grouped[key].action_keluar = true; if (d.keterangan && d.keterangan.toLowerCase().includes('lupa')) grouped[key].isLupa = true; }
            grouped[key].mapel = d.subject || grouped[key].mapel;
        });
        ...
        Object.values(grouped).forEach(g => {
            ...
            if (g.action_masuk && g.jadwal_masuk !== '-') {
                let jM = parseTimeToMins(g.jadwal_masuk); let wM = parseTimeToMins(g.waktu_masuk);
                if (jM > 0 && wM > 0) { let diff = wM - jM; if (diff > tolMasuk) reportMap[g.teacher].telat += (diff - tolMasuk); }
            }
            if (g.action_keluar && g.jadwal_keluar !== '-' && !g.isLupa) {
                let jK = parseTimeToMins(g.jadwal_keluar); let wK = parseTimeToMins(g.waktu_keluar);
                if (jK > 0 && wK > 0) { let diff = jK - wK; if (diff > tolKeluar) reportMap[g.teacher].cepat += (diff - tolKeluar); }
            }
        });
        ...
        Object.values(reportMap).sort((a,b) => a.name.localeCompare(b.name)).forEach(item => {
            const tr = document.createElement('tr');
            tr.innerHTML = `<td class="font-bold">${item.name}</td><td>${item.mapel}</td><td class="text-primary font-bold text-center">${item.total_sesi}</td><td class="text-center ${item.telat > 0 ? 'text-warning font-bold' : ''}">${item.telat}</td><td class="text-center ${item.cepat > 0 ? 'text-danger font-bold' : ''}">${item.cepat}</td><td class="text-center ${item.lupa > 0 ? 'text-purple-600 font-bold' : ''}">${item.lupa}</td>`;
            tbody.appendChild(tr);
        });
        if (Object.keys(reportMap).length === 0) tbody.innerHTML = '<tr><td colspan="6" class="text-center py-4">Tidak ada data terakumulasi.</td></tr>';
    } catch (e) { tbody.innerHTML = `<tr><td colspan="6" class="text-center text-danger py-4">Error: ${e.message}</td></tr>`; }
...
function renderMasterAdminMonitor() {
    const tbody = document.getElementById('listMasterAdminBody'); const totalBadge = document.getElementById('totalGuruMonitor'); if (!tbody) return;
    const todayStr = getLocalDateString(getCurrentWITA()); const serverDataToday = currentServerDataCacheAdmin.filter(d => d.date === todayStr);
    const sortedTeachers = [...TEACHERS].sort((a, b) => a.name.localeCompare(b.name));
    
    totalBadge.textContent = `${sortedTeachers.length} Guru`; let rowsHTML = '';
    sortedTeachers.forEach(t => {
        const lastActivity = [...serverDataToday].reverse().find(d => d.teacher === t.name);
        let statusDisplay = ''; let waktuDisplay = '-';

        if (lastActivity) {
            waktuDisplay = lastActivity.time || '-';
            if (lastActivity.action === 'masuk') {
                statusDisplay = `<span style="display: inline-flex; align-items: center; background-color: #ecfdf5; color: #065f46; padding: 4px 8px; border-radius: 9999px; font-size: 11px; font-weight: 500;"><span style="width: 6px; height: 6px; background-color: #10b981; border-radius: 50%; margin-right: 6px; display: inline-block;"></span>Mengajar Kelas ${lastActivity.class} (${lastActivity.subject || '-'})</span>`;
            } else if (lastActivity.action === 'keluar') {
                statusDisplay = `<span style="display: inline-flex; align-items: center; background-color: #f3f4f6; color: #374151; padding: 4px 8px; border-radius: 9999px; font-size: 11px; font-weight: 500;">Selesai Mengajar Kelas ${lastActivity.class}</span>`;
            }
        } else {
            statusDisplay = `<span style="display: inline-flex; align-items: center; background-color: #fef2f2; color: #991b1b; padding: 4px 8px; border-radius: 9999px; font-size: 11px; font-weight: 500;">Belum Scan Masuk Hari Ini</span>`;
        }
        rowsHTML += `<tr class="tr-monitor" style="border-bottom: 1px solid #e5e7eb;"><td style="padding: 10px; font-size: 12px; font-weight: 600; color: #111827;">${t.name}</td><td style="padding: 10px; font-size: 12px;">${statusDisplay}</td><td style="padding: 10px; font-size: 12px; color: #4b5563; font-family: monospace;">${waktuDisplay}</td></tr>`;
    });
    tbody.innerHTML = rowsHTML || '<tr><td colspan="3" style="text-align: center; padding: 10px;">Tidak ada data guru.</td></tr>';
}

function renderPembiasaanPagiAdmin() {
    const tbody = document.getElementById('bodyPembiasaan'); if (!tbody || currentServerDataCacheAdmin.length === 0) return;
    const period = document.getElementById('filterPembiasaanPeriode').value; const todayStr = getLocalDateString(new Date()); const now = new Date();
    let validDates = []; let startDate = new Date();
    if (period === 'day') startDate = new Date(); if (period === 'week') startDate = new Date(now.setDate(now.getDate() - now.getDay() + 1)); if (period === 'month') startDate = new Date(now.getFullYear(), now.getMonth(), 1);
    
    let tempDate = new Date(startDate);
    while (getLocalDateString(tempDate) <= todayStr) {
        if (tempDate.getDay() !== 0 && !isLiburSekolah(tempDate)) { validDates.push(getLocalDateString(tempDate)); }
        tempDate.setDate(tempDate.getDate() + 1);
    }
    ...
    let html = '';
    for (let guru in WALI_KELAS_MAP) {
        let kelas = WALI_KELAS_MAP[guru]; let records = dataPembiasaan.filter(d => d.teacher === guru);
        let totalHadir = records.length; let alfa = totalHariAktif - totalHadir; let telat = 0;
        records.forEach(r => { let m = parseTimeToMins('07:15'); let w = parseTimeToMins(r.time); if (w > (m + 3)) telat++; });
        html += `<tr><td class="font-bold text-xs">${guru}</td><td class="font-bold text-xs">${kelas}</td><td class="text-success font-bold text-center">${totalHadir}</td><td class="${telat > 0 ? 'text-warning' : 'text-gray-400'} font-bold text-center">${telat}</td><td class="${alfa > 0 ? 'text-danger' : 'text-gray-400'} font-bold text-center">${alfa > 0 ? alfa : 0}</td></tr>`;
    }
    tbody.innerHTML = html;
}

function getWeekDateRange(offsetWeeks = 0) {
    const now = new Date(); const dayOfWeek = now.getDay() || 7; 
    now.setDate(now.getDate() - dayOfWeek + 1); now.setHours(0,0,0,0); now.setDate(now.getDate() - (offsetWeeks * 7)); 
    const startDate = new Date(now); const endDate = new Date(now); endDate.setDate(endDate.getDate() + 5); 
    return { start: startDate, end: endDate };
}

function populateRiwayatWeeks() {
    const select = document.getElementById('riwayatWeekSelect'); if (select.options.length > 0) return; 
    for (let i = 0; i < 5; i++) { let opt = document.createElement('option'); opt.value = i; opt.textContent = i === 0 ? 'Pekan Ini' : `${i} Pekan Lalu`; select.appendChild(opt); }
    select.addEventListener('change', loadRiwayatPekanan);
}
...
async function loadRiwayatPekanan() {
    if (!currentUser) return;
    const container = document.getElementById('riwayatPekanContainer'); document.getElementById('riwayatPekanGuru').textContent = currentUser;
    const offset = parseInt(document.getElementById('riwayatWeekSelect').value) || 0; const range = getWeekDateRange(offset);
    const formatTgl = (d) => `${String(d.getDate()).padStart(2,'0')} ${['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agu','Sep','Okt','Nov','Des'][d.getMonth()]} ${d.getFullYear()}`;
    document.getElementById('riwayatNotice').innerHTML = `<div class="bg-purple-50 border border-purple-200 text-purple-800 p-2 rounded-lg text-xs mb-3 text-center"><i class="fa-solid fa-calendar-days"></i> Periode: <b>${formatTgl(range.start)} - ${formatTgl(range.end)}</b></div>`;
    
    container.innerHTML = '<div class="text-center py-6 font-bold text-gray-500"><i class="fa-solid fa-spinner fa-spin mr-2"></i> Menarik data server...</div>';
    try {
        if (!cachedFullRiwayat) { const response = await fetch(GAS_URL + '?action=getAbsensi'); const res = await response.json(); if (res.success) cachedFullRiwayat = res.data; else throw new Error("Gagal load data"); }
        renderRiwayatData(cachedFullRiwayat, range);
    } catch(e) { container.innerHTML = `<div class="text-center py-6 text-danger"><i class="fa-solid fa-triangle-exclamation"></i> Gagal memuat riwayat: Cek koneksi Anda.</div>`; }
}

function renderRiwayatData(allData, range) {
    const container = document.getElementById('riwayatPekanContainer'); let html = ''; const days = ['senin', 'selasa', 'rabu', 'kamis', 'jumat', 'sabtu']; let totalSesi = 0;
    const startStr = getLocalDateString(range.start); const endStr = getLocalDateString(range.end); const witaNow = getCurrentWITA(); const todayStr = getLocalDateString(witaNow); const now = witaNow; const currentMins = now.getHours() * 60 + now.getMinutes();
    
    const userData = allData.filter(d => { if (d.teacher !== currentUser) return false; let dt = d.date; if (dt && dt.includes('T')) dt = dt.split('T')[0]; return dt >= startStr && dt <= endStr; });
    
    for (let i = 0; i < days.length; i++) {
        let day = days[i]; let currentDate = new Date(range.start); currentDate.setDate(currentDate.getDate() + i); let currentDateStr = getLocalDateString(currentDate);
        let blocks = getGroupedSchedule(currentUser, day, 0, currentDate);
        
        if (blocks.length > 0) {
            let dayName = day.charAt(0).toUpperCase() + day.slice(1); let dateDisplay = `${String(currentDate.getDate()).padStart(2,'0')}/${String(currentDate.getMonth()+1).padStart(2,'0')}`;
            let isFutureDay = currentDateStr > todayStr; let headerBg = isFutureDay ? 'bg-gray-100 text-gray-400 border-gray-200' : 'bg-purple-50 text-purple-700 border-purple-200';
            
            html += `<div class="p-3 rounded-xl border border-gray-200" style="background: var(--input-bg);"><div class="font-bold mb-2 border-b pb-2 flex justify-between items-center px-2 rounded-t-lg ${headerBg}"><span><i class="fa-regular fa-calendar-check"></i> ${dayName}, ${dateDisplay}</span></div><table class="w-full text-sm"><tbody>`;
            
            blocks.forEach(b => {
                let jpStr = b.jps.length > 1 ? `JP ${b.jps[0]}-${b.jps[b.jps.length-1]}` : `JP ${b.jps[0]}`;
                let hasIn = false, hasOut = false; let actualMasuk = '--:--'; let actualKeluar = '--:--';
                
                userData.forEach(h => {
                    let hDate = h.date.includes('T') ? h.date.split('T')[0] : h.date;
                    if (hDate === currentDateStr) {
                        let overlap = false;
                        if (h.jp) { let hJps = h.jp.replace('JP ', '').split('-'); let hStart = parseInt(hJps[0]), hEnd = hJps.length > 1 ? parseInt(hJps[1]) : hStart; b.jps.forEach(j => { if (j >= hStart && j <= hEnd) overlap = true; }); }
                        let classMatch = false; const cleanSchClass = b.class.replace(/[()]/g, ''); const cleanHClass = (h.class || '').replace(/[()]/g, '');
                        if (cleanSchClass === cleanHClass) classMatch = true;
                        else if (b.mapel.toLowerCase().includes('tahfiz') && (h.subject||'').toLowerCase().includes('tahfiz')) { const groups = [['7A','7B','7C'],['7D','7E','8A'],['8B','8C','8D','8E'],['9A','9B','9C','9D']]; for (let g of groups) { if (g.some(c => cleanSchClass.includes(c)) && g.some(c => cleanHClass.includes(c))) classMatch = true; } }
                        
                        if (overlap && classMatch) { if (h.action === 'masuk') { hasIn = true; actualMasuk = h.time ? h.time.substring(0, 5) : '--:--'; } if (h.action === 'keluar') { hasOut = true; actualKeluar = h.time ? h.time.substring(0, 5) : '--:--'; } }
                    }
                });
                
                let isPast = currentDateStr < todayStr || (currentDateStr === todayStr && b.kMins < currentMins); let statusIcon = '';
                if (hasIn && hasOut) { statusIcon = `<span class="text-success text-xl" title="Selesai (In & Out)"><i class="fa-solid fa-check-double"></i></span>`; } 
                else if (hasIn && !hasOut) { statusIcon = `<span class="text-primary text-xl" title="Lupa Scan Out"><i class="fa-solid fa-check"></i></span>`; } 
                else if (!hasIn && isPast) { statusIcon = `<span class="text-danger text-xl opacity-75" title="Terlewat / Alpa"><i class="fa-solid fa-xmark"></i></span>`; } 
                else { statusIcon = `<span class="text-gray-300 text-xl" title="Belum Mulai">-</span>`; }
                
                html += `<tr class="border-b last:border-0 border-gray-100"><td class="py-2 pr-2 font-bold text-gray-600 align-middle whitespace-nowrap" style="width: 25%; font-size: 0.75rem;">${jpStr}<br><span class="text-xs font-bold ${hasIn || hasOut ? 'text-primary' : 'text-muted'}">${actualMasuk} - ${actualKeluar}</span></td><td class="py-2 pr-2 align-middle"><div class="font-black text-gray-800">${b.class}</div><div class="text-purple-600 font-bold" style="font-size: 0.75rem;">${b.mapel}</div></td><td class="py-2 text-center align-middle" style="width: 20%;">${statusIcon}</td></tr>`;
                totalSesi++;
            });
            html += `</tbody></table></div>`;
        }
...
function renderJadwalPekanIni() {
    if (!currentUser) return;
    const container = document.getElementById('jadwalPekanContainer'); const guruText = document.getElementById('jadwalPekanGuru'); const badge = document.getElementById('jadwalPekanBadge');
    guruText.textContent = currentUser;
    const isGenap = isEvenWeek(); badge.innerHTML = isGenap ? '<i class="fa-solid fa-2"></i> Pekan Genap' : '<i class="fa-solid fa-1"></i> Pekan Ganjil';
    
    let html = ''; const days = ['senin', 'selasa', 'rabu', 'kamis', 'jumat', 'sabtu']; let totalSesi = 0;
    const range = getWeekDateRange(0);

    days.forEach((day, index) => {
        let currentDate = new Date(range.start); currentDate.setDate(currentDate.getDate() + index);
        const statusHari = getStatusHariIni(currentDate); let blocks = getGroupedSchedule(currentUser, day, 0, currentDate);
        let dayName = day.charAt(0).toUpperCase() + day.slice(1); let dateDisplay = `${String(currentDate.getDate()).padStart(2,'0')}/${String(currentDate.getMonth()+1).padStart(2,'0')}`;
        let borderStyle = 'border-gray-200'; let bgStyle = 'var(--input-bg)'; let badgeInfo = '';

        if (statusHari.status === 'libur') { borderStyle = 'border-red-400'; bgStyle = '#fef2f2'; badgeInfo = `<span class="badge bg-red-100 text-red-700 font-bold px-2 py-1 text-xs"><i class="fa-solid fa-triangle-exclamation"></i> Libur: ${statusHari.label}</span>`; } 
        else if (statusHari.status === 'kegiatan') { borderStyle = 'border-emerald-400'; bgStyle = '#ecfdf5'; badgeInfo = `<span class="badge bg-emerald-100 text-emerald-700 font-bold px-2 py-1 text-xs"><i class="fa-solid fa-calendar-check"></i> ${statusHari.label}</span>`; }

        html += `<div class="p-3 rounded-xl border-2 ${borderStyle} mb-3" style="background: ${bgStyle};"><div class="font-bold text-primary mb-2 border-b border-gray-200 pb-1 flex justify-between items-center"><span><i class="fa-regular fa-calendar-days"></i> ${dayName}, ${dateDisplay}</span>${badgeInfo}</div>`;

        if (statusHari.status === 'libur') { html += `<div class="text-center py-3 text-red-600 font-bold text-xs"><i class="fa-solid fa-mug-hot">></i> Hari Tidak Mengajar (Libur Tanggal Merah / Semester)</div>`; } 
        else if (blocks.length > 0) {
            html += `<table class="w-full text-sm"><tbody>`;
            blocks.forEach(b => { let jpStr = b.jpStr; html += `<tr class="border-b last:border-0 border-gray-200"><td class="py-2 pr-2 font-bold text-gray-700 align-top whitespace-nowrap" style="width: 25%; font-size: 0.75rem;">${jpStr}<br><span class="text-xs text-muted font-normal">${b.masuk} - ${b.keluar}</span></td><td class="py-2 pr-2 font-black text-gray-800 align-top" style="width: 20%;">${b.class}</td><td class="py-2 text-primary font-bold align-top" style="font-size: 0.8rem;">${b.mapel}</td></tr>`; totalSesi++; });
            html += `</tbody></table>`;
        } else { html += `<div class="text-center py-2 text-muted text-xs">Tidak ada jadwal mengajar pada hari ini.</div>`; }
        html += `</div>`;
    });
    container.innerHTML = html;
}
...
// ==========================================
// BLOK 7: EXPORT WA/EXCEL, ANIMASI & EVENT LISTENER
// ==========================================
function exportTableToExcel(tableId, filename = 'Data_Absensi.xls') {
    const table = document.getElementById(tableId); if (!table) return;
    const template = `<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><meta charset="UTF-8"></head><body><table>${table.cloneNode(true).innerHTML}</table></body></html>`;
...
function copyRekapWA() {
    const tbody = document.getElementById('missedClassesBody'); const rows = tbody.querySelectorAll('tr');
    const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }; const tanggalSekarang = new Date().toLocaleDateString('id-ID', options);
    let textToCopy = "*REKAP GURU TIDAK SCAN MASUK HARI INI*\n*SMP Islam Terpadu Putra Abu Hurairah Mataram*\nTanggal: " + tanggalSekarang + "\n\n";
    
    if (rows.length === 0 || (rows.length === 1 && (rows[0].innerText.includes('Menghitung') || rows[0].innerText.includes('Tidak ada')))) { textToCopy += "Alhamdulillah, tidak ada catatan guru yang tidak scan masuk hari ini."; } 
    else {
        let index = 1;
        rows.forEach(row => {
            const cells = row.querySelectorAll('td');
            if (cells.length >= 5) { textToCopy += `${index}. ${cells[0].innerText.trim()}\n   📚 Kls: ${cells[1].innerText.trim()} | ${cells[2].innerText.trim()} | ${cells[3].innerText.trim()}\n\n`; index++; }
        });
    }
    navigator.clipboard.writeText(textToCopy).then(() => { if (typeof showToast === 'function') { showToast("Rekap berhasil disalin untuk WhatsApp!", "success"); } else { alert("Rekap berhasil disalin untuk WhatsApp!"); } }).catch(err => { console.error('Gagal menyalin teks', err); alert("Gagal menyalin teks. Silakan coba lagi."); });
}

window.copyRekapPribadiWA = function() {
    const tbody = document.getElementById('rekapBody'); const rows = tbody.querySelectorAll('tr');
    const userDisplay = document.getElementById('userNameDisplay').textContent; const periodeDisplay = document.getElementById('rekapPeriode').textContent;
    let textToCopy = `*REKAP ABSENSI PRIBADI*\n👤 Guru: ${userDisplay}\n📅 Periode: ${periodeDisplay}\n\n`;
    
    if (rows.length === 0 || (rows.length === 1 && rows[0].innerText.includes('Belum ada data'))) { textToCopy += "Belum ada data absensi pada periode ini."; } 
    else {
        rows.forEach((row, index) => {
            const cells = row.querySelectorAll('td');
            if (cells.length >= 7) {
                textToCopy += `${index + 1}. Tanggal: ${cells[0].innerText.trim()}\n   📚 Kls: ${cells[1].innerText.trim()} (${cells[2].innerText.trim()})\n   ⏰ In: ${cells[3].innerText.trim()} | Out: ${cells[4].innerText.trim()}\n   📌 Status: ${cells[5].innerText.replace(/\n/g, ' | ').trim()}\n`;
                let ket = cells[6].innerText.trim(); if(ket && ket !== '-') textToCopy += `   📝 Ket: ${ket}\n`; textToCopy += `\n`;
            }
        });
        textToCopy += `*Ringkasan:*\nTotal Sesi: ${document.getElementById('statTotal').textContent} | Total Telat: ${document.getElementById('statLate').textContent} Menit`;
    }
    navigator.clipboard.writeText(textToCopy).then(() => { if (typeof showToast === 'function') { showToast("Rekap pribadi berhasil disalin!", "success"); } else { alert("Rekap berhasil disalin!"); } }).catch(err => { console.error('Gagal menyalin teks', err); });
};

window.copyRekapNilaiWA = function() {
    const filterKelas = document.getElementById('filterRekapNilaiKelas').value; const filterJenis = document.getElementById('filterRekapNilaiJenis').value;
    const rows = document.getElementById('rekapNilaiBody').querySelectorAll('tr');
    let textToCopy = `*REKAP NILAI SANTRI*\n👨‍🏫 Guru: ${currentUser}\n🏫 Kelas: ${filterKelas === 'ALL' ? 'Semua Kelas' : filterKelas}\n📝 Jenis: ${filterJenis === 'ALL' ? 'Semua Penilaian' : filterJenis}\n\n`;
    
    if (rows.length === 0 || rows[0].innerText.includes('Belum ada')) { textToCopy += "Belum ada data nilai."; } 
    else {
        let currentUjian = "";
        rows.forEach((row) => {
            const cells = row.querySelectorAll('td');
            if (cells.length >= 3) {
                let namaUjian = cells[0].innerText.split('\n')[0].trim(); let namaSantri = cells[1].innerText.trim(); let skor = cells[2].innerText.trim();
                if (namaUjian !== currentUjian) { textToCopy += `\n*=== ${namaUjian.toUpperCase()} ===*\n`; currentUjian = namaUjian; }
                textToCopy += `• ${namaSantri} : *${skor}*\n`;
            }
        });
    }
    navigator.clipboard.writeText(textToCopy).then(() => { showToast("Rekap Nilai disalin ke WhatsApp!", "success"); }).catch(err => { alert("Gagal menyalin. Silakan coba lagi."); });
};

async function clearAppCacheAndReload() {
    try {
        if ('caches' in window) { const cacheNames = await caches.keys(); await Promise.all(cacheNames.map(name => caches.delete(name))); }
        if ('serviceWorker' in navigator) { const registrations = await navigator.serviceWorker.getRegistrations(); for (let registration of registrations) { await registration.unregister(); } }
        showToast('Memperbaiki error scanner. Memuat ulang aplikasi...', 'warning');
        setTimeout(() => { window.location.reload(true); }, 1500);
    } catch (e) { console.error('Gagal menghapus cache:', e); window.location.reload(true); }
}

function applySkin(skinName) {
    document.body.classList.remove('dark-skin', 'green-skin', 'purple-skin');
    if (skinName !== 'light') { document.body.classList.add(skinName + '-skin'); }
}
...
doLogin = async function(name, isAdminUser, adminType) {
    await originalDoLogin(name, isAdminUser, adminType);
    const klsWali = WALI_KELAS_MAP[name];
    if (klsWali || adminType === 'agus') { 
        document.getElementById('waliTab').classList.remove('hidden'); 
        document.getElementById('waliKelasBadge').textContent = klsWali ? 'Kelas: ' + klsWali : 'Akses Pantauan Master'; 
        document.getElementById('tglAbsensiSantri').value = getLocalDateString(new Date()); 
    } else { document.getElementById('waliTab').classList.add('hidden'); }
};

// Event Listeners Dasar
document.getElementById('adminLoginBtn').addEventListener('click', (e) => {
    e.preventDefault(); const section = document.getElementById('passwordSection');
    const teacherName = section.dataset.selectedTeacher, password = document.getElementById('adminPassword').value.trim();
    const adminType = section.dataset.adminType || '';
    let expectedPassword = adminType === 'agus' ? 'a133' : ({'senin':'seni212', 'selasa':'selas212', 'rabu':'rab212', 'kamis':'kami212', 'jumat':'juma212', 'sabtu':'sabt212', 'ahad':'aha212'})[getDayName()];
    if (password === expectedPassword) doLogin(teacherName, true, adminType); else { showToast('Password Salah!', 'error'); document.getElementById('adminPassword').value = ''; }
});
document.getElementById('teacherOnlyLoginBtn').addEventListener('click', (e) => { e.preventDefault(); doLogin(document.getElementById('passwordSection').dataset.selectedTeacher, false, ''); });
document.getElementById('adminPassword').addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); document.getElementById('adminLoginBtn').click(); } });
document.getElementById('switchUserBtn').addEventListener('click', () => {
    stopScanner(); document.getElementById('app').classList.add('hidden'); document.getElementById('loginScreen').classList.remove('hidden'); document.getElementById('passwordSection').classList.add('hidden');
    document.querySelectorAll('#teacherList .btn').forEach(b => b.classList.remove('selected'));
    currentUser = null; currentUserData = null; isAdmin = false; isAgus = false;
    localStorage.removeItem('absensi_user'); localStorage.removeItem('absensi_admin'); localStorage.removeItem('absensi_admin_type');
});
...
document.getElementById('manualScanBtn').addEventListener('click', () => {
    const area = document.getElementById('manualScanArea'); const select = document.getElementById('manualClassSelect');
    if (area.classList.contains('hidden')) {
        select.innerHTML = '<option value="">-- Pilih Kelas --</option>'; const rendered = new Set();
        CLASSES.forEach(cls => { const mockSubject = (currentUser === 'M. Tahir, M. Pd.') ? 'PJOK' : 'Mapel'; const formatted = formatKopelClass(cls, mockSubject); if (!rendered.has(formatted)) { rendered.add(formatted); const opt = document.createElement('option'); opt.value = cls; opt.textContent = formatted; select.appendChild(opt); } });
        area.classList.remove('hidden');
    } else { area.classList.add('hidden'); }
});
document.getElementById('manualProcessBtn').addEventListener('click', () => { const val = document.getElementById('manualClassSelect').value.trim().toUpperCase(); if (val) { processScan(val); document.getElementById('manualScanArea').classList.add('hidden'); } else { showToast('Silakan pilih kelas yang valid', 'error'); } });

document.getElementById('syncNowBtn').addEventListener('click', () => syncNow(true, true));
document.getElementById('adminSyncNow').addEventListener('click', () => syncNow(true, true));
document.getElementById('agusSyncNow').addEventListener('click', () => syncNow(true, true));

document.getElementById('jenisIzinSelect').addEventListener('change', function() { if (this.value === 'Lainnya') { document.getElementById('keteranganLainGroup').classList.remove('hidden'); } else { document.getElementById('keteranganLainGroup').classList.add('hidden'); document.getElementById('keteranganLainInput').value = ''; } });
const searchSantriIzinInput = document.getElementById('searchSantriIzinInput'); const santriIzinDropdown = document.getElementById('santriIzinDropdown');
searchSantriIzinInput.addEventListener('input', function() {
    let keyword = this.value.trim().toLowerCase(); santriIzinDropdown.innerHTML = '';
    if (!keyword) { santriIzinDropdown.classList.add('hidden'); return; }
    let activeTeaching = JSON.parse(localStorage.getItem('activeTeaching_' + currentUser) || "null"); if (!activeTeaching) { santriIzinDropdown.classList.add('hidden'); return; }
    let targetClass = activeTeaching.rawClass; if (!targetClass) { CLASSES.forEach(c => { if(activeTeaching.class.includes(c)) targetClass = c; }); }
    let listSantriKelas = DATA_SANTRI[targetClass] || []; let matched = listSantriKelas.filter(nama => nama.toLowerCase().includes(keyword));
    if (matched.length > 0) { santriIzinDropdown.classList.remove('hidden'); matched.forEach(nama => { let div = document.createElement('div'); div.className = 'dropdown-item'; div.textContent = nama; div.onclick = () => { searchSantriIzinInput.value = nama; santriIzinDropdown.classList.add('hidden'); }; santriIzinDropdown.appendChild(div); }); } else { santriIzinDropdown.classList.add('hidden'); }
});

const searchSantriKasusInput = document.getElementById('searchSantriKasusInput'); const santriKasusDropdown = document.getElementById('santriKasusDropdown');
searchSantriKasusInput.addEventListener('input', function() {
    let keyword = this.value.trim().toLowerCase(); santriKasusDropdown.innerHTML = '';
    if (!keyword) { santriKasusDropdown.classList.add('hidden'); return; }
    let activeTeaching = JSON.parse(localStorage.getItem('activeTeaching_' + currentUser) || "null"); if (!activeTeaching) { santriKasusDropdown.classList.add('hidden'); return; }
    let targetClass = activeTeaching.rawClass; if (!targetClass) { CLASSES.forEach(c => { if(activeTeaching.class.includes(c)) targetClass = c; }); }
    let listSantriKelas = DATA_SANTRI[targetClass] || []; let matched = listSantriKelas.filter(nama => nama.toLowerCase().includes(keyword));
    if (matched.length > 0) { santriKasusDropdown.classList.remove('hidden'); matched.forEach(nama => { let div = document.createElement('div'); div.className = 'dropdown-item'; div.textContent = nama; div.onclick = () => { searchSantriKasusInput.value = nama; santriKasusDropdown.classList.add('hidden'); }; santriKasusDropdown.appendChild(div); }); } else { santriKasusDropdown.classList.add('hidden'); }
});
document.addEventListener('click', (e) => { if (!searchSantriIzinInput.contains(e.target) && !santriIzinDropdown.contains(e.target)) { santriIzinDropdown.classList.add('hidden'); } if (!searchSantriKasusInput.contains(e.target) && !santriKasusDropdown.contains(e.target)) { santriKasusDropdown.classList.add('hidden'); } });

document.getElementById('tambahSantriIzinBtn').addEventListener('click', () => {
    const nama = searchSantriIzinInput.value.trim(); const jenis = document.getElementById('jenisIzinSelect').value; const ketLain = document.getElementById('keteranganLainInput').value.trim();
    if (!nama) { showToast('Pilih atau ketik nama santri terlebih dahulu', 'error'); return; }
    let jenisFinal = jenis === 'Lainnya' ? `Lainnya (${ketLain || '-'})` : jenis; tempSantriIzinList.push({ nama: nama, jenis_izin: jenisFinal, keterangan_lain: ketLain }); renderSantriIzinTable();
    searchSantriIzinInput.value = ''; document.getElementById('jenisIzinSelect').value = 'Ke WC'; document.getElementById('keteranganLainGroup').classList.add('hidden'); document.getElementById('keteranganLainInput').value = ''; showToast('Santri ditambahkan ke daftar izin', 'success');
});
document.getElementById('tambahSantriKasusBtn').addEventListener('click', () => {
    const nama = searchSantriKasusInput.value.trim(); const jenisKasus = document.getElementById('jenisKasusSelect').value; const ketKasus = document.getElementById('keteranganKasusInput').value.trim();
    if (!nama) { showToast('Pilih atau ketik nama santri terlebih dahulu', 'error'); return; }
    tempSantriKasusList.push({ nama: nama, jenis_kasus: jenisKasus, keterangan_kasus: ketKasus }); renderSantriKasusTable();
    searchSantriKasusInput.value = ''; document.getElementById('jenisKasusSelect').value = 'Bolos'; document.getElementById('keteranganKasusInput').value = ''; showToast('Kasus santri ditambahkan ke daftar', 'success');
});
document.getElementById('kirimSantriIzinBtn').addEventListener('click', async () => {
    let activeTeaching = JSON.parse(localStorage.getItem('activeTeaching_' + currentUser) || "null"); if (!activeTeaching) { showToast('Tidak ada kelas aktif saat ini', 'error'); return; } if (tempSantriIzinList.length === 0) return; if (!navigator.onLine) { showToast('Offline, gagal mengirim data izin', 'error'); return; }
    const now = new Date(); const payload = { api_action: 'addSantriIzin', teacher: currentUser, class: activeTeaching.class, subject: activeTeaching.subject, date: now.toISOString().split('T')[0], time: now.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' }), timestamp: now.toISOString(), santri_list: tempSantriIzinList };
    const btn = document.getElementById('kirimSantriIzinBtn'); btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Mengirim...';
    try { const response = await fetch(GAS_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify(payload) }); const res = await response.json(); if (res.success) { showToast('✅ Data santri izin berhasil dikirim ke Spreadsheet!', 'success'); tempSantriIzinList = []; renderSantriIzinTable(); } else { throw new Error(res.error || 'Gagal menyimpan'); } } catch (e) { showToast('❌ Gagal kirim: ' + e.message, 'error'); } finally { btn.innerHTML = '<i class="fa-solid fa-paper-plane"></i> Kirim Data Izin ke Server'; }
});
document.getElementById('kirimSantriKasusBtn').addEventListener('click', async () => {
    let activeTeaching = JSON.parse(localStorage.getItem('activeTeaching_' + currentUser) || "null"); if (!activeTeaching) { showToast('Tidak ada kelas aktif saat ini', 'error'); return; } if (tempSantriKasusList.length === 0) return; if (!navigator.onLine) { showToast('Offline, gagal mengirim data kasus', 'error'); return; }
    const now = new Date(); const payload = { api_action: 'addSantriKasus', teacher: currentUser, class: activeTeaching.class, subject: activeTeaching.subject, date: now.toISOString().split('T')[0], time: now.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' }), timestamp: now.toISOString(), kasus_list: tempSantriKasusList };
    const btn = document.getElementById('kirimSantriKasusBtn'); btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Mengirim...';
    try { const response = await fetch(GAS_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify(payload) }); const res = await response.json(); if (res.success) { showToast('✅ Data kasus santri berhasil dikirim ke Spreadsheet!', 'success'); tempSantriKasusList = []; renderSantriKasusTable(); } else { throw new Error(res.error || 'Gagal menyimpan'); } } catch (e) { showToast('❌ Gagal kirim: ' + e.message, 'error'); } finally { btn.innerHTML = '<i class="fa-solid fa-paper-plane"></i> Kirim Data Kasus ke Server'; }
});

// Event Listener Tampilan Tab
document.querySelectorAll('#mainTabs .tab').forEach(tab => {
    tab.addEventListener('click', () => {
        document.querySelectorAll('#mainTabs .tab').forEach(t => t.classList.remove('active')); tab.classList.add('active');
        document.querySelectorAll('.tab-content').forEach(el => el.classList.add('hidden'));
        ...
        if (tab.dataset.tab === 'admin' && isAdmin) loadAdminData();
        if (tab.dataset.tab === 'agus' && isAgus) loadAgusData();
        if (tab.dataset.tab === 'riwayat') { populateRiwayatWeeks(); loadRiwayatPekanan(); }
    });
});
...
document.querySelectorAll('.rekap-filter').forEach(btn => {
    btn.addEventListener('click', () => {
        document.querySelectorAll('.rekap-filter').forEach(b => { b.classList.remove('active'); b.classList.replace('btn-primary', 'btn-outline'); });
        btn.classList.add('active'); btn.classList.replace('btn-outline', 'btn-primary');
        rekapFilter = btn.dataset.filter; document.getElementById('rekapPeriode').textContent = btn.textContent.trim(); loadRekap();
    });
});
document.querySelectorAll('.admin-view').forEach(btn => {
    btn.addEventListener('click', () => {
        document.querySelectorAll('.admin-view').forEach(b => { b.classList.remove('active', 'btn-primary'); b.classList.add('btn-outline'); });
        btn.classList.remove('btn-outline'); btn.classList.add('active', 'btn-primary'); adminViewMode = btn.dataset.view; loadAdminData();
    });
});
document.addEventListener('click', function(e) {
    if(e.target && e.target.closest('.live-toggle')) {
        const btn = e.target.closest('.live-toggle');
        document.querySelectorAll('.live-toggle').forEach(b => { b.classList.remove('btn-primary'); b.classList.add('btn-outline'); });
        btn.classList.remove('btn-outline'); btn.classList.add('btn-primary'); liveMonitorMode = btn.dataset.mode; renderLiveMonitor();
    }
});
...
document.getElementById('adminRefresh').addEventListener('click', loadAdminData);
document.getElementById('saveToleranceBtn').addEventListener('click', () => {
    localStorage.setItem('tolMasuk', document.getElementById('toleranceMasukInput').value); localStorage.setItem('tolKeluar', document.getElementById('toleranceKeluarInput').value);
    showToast('Konfigurasi Toleransi Disimpan!', 'success'); if (isAdmin) loadAdminData(); if (isAgus) loadAgusData();
});
document.getElementById('agusSyncJadwal').addEventListener('click', async () => {
    const btn = document.getElementById('agusSyncJadwal'); const originalText = btn.innerHTML; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Menarik Jadwal...'; addSyncLog('Tarik data jadwal dari server...', 'info');
    try {
        const response = await fetch(GAS_URL + '?action=getJadwal'); if (!response.ok) throw new Error("Gagal terhubung ke GAS");
        const result = await response.json();
        if (result.success && result.data) {
            result.data.forEach(row => {
                const vKelas = row[0], vHari = (row[1] || "").toString().toLowerCase().trim(), vJp = (row[2] || "").toString().toUpperCase().trim(), vMapel = row[3], vGuru = row[4];
            ...
            localStorage.setItem('jadwal_cache', JSON.stringify(jadwalPelajaran)); addSyncLog('✅ Jadwal berhasil diperbarui', 'success'); showToast('Jadwal disinkronkan', 'success');
        } else { addSyncLog('⚠️ Format JSON tidak sesuai', 'error'); }
    } catch (e) { showToast('Gagal Tarik Jadwal', 'error'); addSyncLog('❌ ' + e.message, 'error'); } btn.innerHTML = originalText;
});
document.getElementById('agusCheckConnection').addEventListener('click', async () => {
    addSyncLog('📡 Cek ping server...', 'info');
    try { const response = await fetch(GAS_URL + '?action=ping'); if (response.ok) { showToast('Ping OK!', 'success'); addSyncLog('✅ Ping Berhasil', 'success'); } else { showToast('Ping Gagal', 'error'); addSyncLog('❌ Status: ' + response.status, 'error'); } } catch (e) { showToast('Network Error', 'error'); addSyncLog('❌ Error network', 'error'); }
});
document.getElementById('agusLoadReport').addEventListener('click', loadAgusData);
document.getElementById('agusEditLink').addEventListener('click', () => {
    document.getElementById('modalContent').innerHTML = `<div class="input-group"><label>URL Apps Script Terpasang</label><input type="text" id="gasUrlInput" value="${GAS_URL}" /></div><button class="btn btn-primary" id="saveGasUrl">Simpan Link</button>`; document.getElementById('modalOverlay').classList.remove('hidden');
    document.getElementById('saveGasUrl').onclick = () => { GAS_URL = document.getElementById('gasUrlInput').value.trim(); localStorage.setItem('GAS_URL', GAS_URL); showToast('Link Disimpan!', 'success'); document.getElementById('modalOverlay').classList.add('hidden'); };
});
document.getElementById('agusGenQR').addEventListener('click', () => { let html = '<div class="grid-2">'; CLASSES.forEach(cls => { html += `<div class="card text-center p-2"><h2 class="text-primary font-bold text-2xl">${cls}</h2></div>`; }); html += '</div>'; document.getElementById('modalContent').innerHTML = html; document.getElementById('modalOverlay').classList.remove('hidden'); });
document.getElementById('modalClose').addEventListener('click', () => { document.getElementById('modalOverlay').classList.add('hidden'); });

window.addEventListener('online', async () => { updateOfflineStatus(); showToast('🌐 Online: Sync otomatis berjalan...', 'success'); await syncNow(false, true); await verifySheetStatus(); });
window.addEventListener('offline', () => { updateOfflineStatus(); showToast('📴 Offline', 'warning'); });
setInterval(() => { if (navigator.onLine) syncNow(false, false); }, 30000);

const savedSkin = localStorage.getItem('app_skin') || 'light'; applySkin(savedSkin);
const skinBtnToggle = document.getElementById('skinBtnToggle'); const skinDropdown = document.getElementById('skinDropdown');
if (skinBtnToggle) { skinBtnToggle.addEventListener('click', (e) => { e.stopPropagation(); skinDropdown.classList.toggle('hidden'); }); }
document.querySelectorAll('#skinDropdown .dropdown-item').forEach(item => { item.addEventListener('click', (e) => { const selectedSkin = e.target.closest('.dropdown-item').dataset.skin; applySkin(selectedSkin); localStorage.setItem('app_skin', selectedSkin); skinDropdown.classList.add('hidden'); showToast('Tema aplikasi berhasil diubah', 'success'); }); });
document.addEventListener('click', (e) => { if (skinDropdown && !skinDropdown.contains(e.target) && e.target !== skinBtnToggle) { skinDropdown.classList.add('hidden'); } });

document.querySelectorAll('.tab-wali').forEach(btn => {
    btn.addEventListener('click', () => {
        document.querySelectorAll('.tab-wali').forEach(b => b.classList.replace('btn-primary', 'btn-outline')); btn.classList.replace('btn-outline', 'btn-primary');
        document.getElementById('waliAbsensiSantri').classList.add('hidden'); document.getElementById('waliRekapPembiasaan').classList.add('hidden'); document.getElementById(btn.dataset.target).classList.remove('hidden');
        if(btn.dataset.target === 'waliRekapPembiasaan') renderPembiasaanWali();
    });
});
const searchAbsensiSantri = document.getElementById('searchAbsensiSantri'); const dropdownAbsensiSantri = document.getElementById('dropdownAbsensiSantri');
searchAbsensiSantri.addEventListener('input', function() {
    let keyword = this.value.trim().toLowerCase(); dropdownAbsensiSantri.innerHTML = ''; let kls = WALI_KELAS_MAP[currentUser];
    if (!keyword || !kls) { dropdownAbsensiSantri.classList.add('hidden'); return; }
    let matched = (DATA_SANTRI[kls] || []).filter(n => n.toLowerCase().includes(keyword));
    if (matched.length > 0) { dropdownAbsensiSantri.classList.remove('hidden'); matched.forEach(nama => { let div = document.createElement('div'); div.className = 'dropdown-item'; div.textContent = nama; div.onclick = () => { searchAbsensiSantri.value = nama; dropdownAbsensiSantri.classList.add('hidden'); }; dropdownAbsensiSantri.appendChild(div); }); } else { dropdownAbsensiSantri.classList.add('hidden'); }
});
document.getElementById('btnKirimAbsensiSantri').addEventListener('click', async () => {
    const tgl = document.getElementById('tglAbsensiSantri').value; const nama = searchAbsensiSantri.value.trim(); const status = document.getElementById('statusAbsensiSantri').value; const kls = WALI_KELAS_MAP[currentUser];
    if (!kls) return showToast('Anda bukan wali kelas!', 'error'); if (!tgl || !nama) return showToast('Pilih tanggal dan nama santri!', 'error');
    const btn = document.getElementById('btnKirimAbsensiSantri'); const oriText = btn.innerHTML; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Mengirim...';
    try { const payload = { api_action: 'addAbsensiSantriHarian', teacher: currentUser, class: kls, date: tgl, nama_santri: nama, status: status }; const res = await fetch(GAS_URL, { method: 'POST', body: JSON.stringify(payload) }); const j = await res.json(); if(j.success) { showToast('Absensi santri berhasil dikirim ke server!', 'success'); searchAbsensiSantri.value = ''; } else { throw new Error(j.error); } } catch(e) { showToast('Gagal mengirim absensi santri.', 'error'); } finally { btn.innerHTML = oriText; }
});

// Event Listener Tab Nilai
document.querySelector('[data-tab="nilai"]').addEventListener('click', () => { initNilaiTab(); });
document.getElementById('nilaiKelasSelect').addEventListener('change', function() {
    const selectedKls = this.value; const mapelSelect = document.getElementById('nilaiMapelInput');
    if (selectedKls && currentGuruScheduleData && currentGuruScheduleData.classesMapping[selectedKls]) {
        const availableMapel = currentGuruScheduleData.classesMapping[selectedKls]; mapelSelect.innerHTML = '<option value="">-- Pilih Mapel --</option>';
        availableMapel.forEach(mpl => { mapelSelect.innerHTML += `<option value="${mpl}">${mpl}</option>`; }); if (availableMapel.length === 1) { mapelSelect.value = availableMapel[0]; }
    }
});
const searchSantriNilaiInput = document.getElementById('searchSantriNilaiInput'); const santriNilaiDropdown = document.getElementById('santriNilaiDropdown');
searchSantriNilaiInput.addEventListener('input', function() {
    let keyword = this.value.trim().toLowerCase(); let selectedClass = document.getElementById('nilaiKelasSelect').value; let selectedMapel = document.getElementById('nilaiMapelInput').value; 
    santriNilaiDropdown.innerHTML = '';
    if (!keyword || !selectedClass) { santriNilaiDropdown.classList.add('hidden'); if (!selectedClass && keyword) showToast('Silakan pilih kelas terlebih dahulu!', 'warning'); return; }
    let listSantriKelas = getDaftarSantriKopel(selectedClass, selectedMapel); let matched = listSantriKelas.filter(nama => nama.toLowerCase().includes(keyword));
    if (matched.length > 0) { santriNilaiDropdown.classList.remove('hidden'); matched.forEach(nama => { let div = document.createElement('div'); div.className = 'dropdown-item'; div.textContent = nama; div.onclick = () => { searchSantriNilaiInput.value = nama; santriNilaiDropdown.classList.add('hidden'); document.getElementById('nilaiAngkaInput').focus(); }; santriNilaiDropdown.appendChild(div); }); } else { santriNilaiDropdown.classList.add('hidden'); }
});
document.addEventListener('click', (e) => { if (!searchSantriNilaiInput.contains(e.target) && !santriNilaiDropdown.contains(e.target)) { santriNilaiDropdown.classList.add('hidden'); } });
document.getElementById('tambahNilaiBtn').addEventListener('click', () => {
    const kls = document.getElementById('nilaiKelasSelect').value; const mapel = document.getElementById('nilaiMapelInput').value.trim(); const tgl = document.getElementById('nilaiTanggalInput').value; const jenis = document.getElementById('nilaiJenisSelect').value; let namaUjian = document.getElementById('nilaiNamaPenilaianInput').value.trim(); const namaSantri = searchSantriNilaiInput.value.trim(); const nilaiAngka = document.getElementById('nilaiAngkaInput').value;
    if (!kls || !mapel) { showToast('Pilih Kelas dan isi Mapel!', 'error'); return; } if (!namaSantri) { showToast('Pilih nama santri terlebih dahulu!', 'error'); return; } if (nilaiAngka === '') { showToast('Masukkan angka nilai!', 'error'); return; } if (!namaUjian) namaUjian = jenis;
    tempNilaiList.push({ tanggal_penilaian: tgl, jenis_nilai: jenis, nama_penilaian: namaUjian, nama_santri: namaSantri, nilai: nilaiAngka });
    renderTempNilaiTable(); searchSantriNilaiInput.value = ''; document.getElementById('nilaiAngkaInput').value = ''; searchSantriNilaiInput.focus(); showToast('Berhasil ditambahkan ke draf', 'success');
});
document.getElementById('kirimNilaiBtn').addEventListener('click', async () => {
    if (tempNilaiList.length === 0) return; if (!navigator.onLine) { showToast('Offline, gagal mengirim data nilai', 'error'); return; }
    const kls = document.getElementById('nilaiKelasSelect').value; const mapel = document.getElementById('nilaiMapelInput').value.trim(); const now = new Date();
    const payload = { api_action: 'addNilai', teacher: currentUser, class: kls, subject: mapel, timestamp: now.toISOString(), nilai_list: tempNilaiList };
    const btn = document.getElementById('kirimNilaiBtn'); const originalText = btn.innerHTML; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Menyimpan ke Spreadsheet...';
    try { const response = await fetch(GAS_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify(payload) }); const res = await response.json(); if (res.success) { showToast('✅ Data nilai berhasil disimpan ke Server!', 'success'); tempNilaiList = []; renderTempNilaiTable(); document.getElementById('nilaiNamaPenilaianInput').value = ''; loadRekapNilai(); } else { throw new Error(res.error || 'Gagal menyimpan'); } } catch (e) { showToast('❌ Gagal kirim: ' + e.message, 'error'); } finally { btn.innerHTML = originalText; }
});
document.getElementById('refreshRekapNilaiBtn').addEventListener('click', loadRekapNilai);
document.getElementById('filterRekapNilaiKelas').addEventListener('change', loadRekapNilai);
document.getElementById('filterRekapNilaiJenis').addEventListener('change', loadRekapNilai);
...
document.addEventListener('DOMContentLoaded', async () => {
    await openDB(); renderTeacherList(); 
    document.getElementById('toleranceMasukInput').value = localStorage.getItem('tolMasuk') || 3;
    document.getElementById('toleranceKeluarInput').value = localStorage.getItem('tolKeluar') || 3;
    document.getElementById('loginDayInfo').textContent = 'Hari ini: ' + getDayNameID() + ', ' + new Date().toLocaleDateString('id-ID');
    document.getElementById('jpDisplay').textContent = getCurrentJPInfo(); 
    ...
    if (typeof updateTanggalIslami === 'function') updateTanggalIslami();
    if (typeof fetchJadwalSholat === 'function') fetchJadwalSholat();
    
    const savedUser = localStorage.getItem('absensi_user'); 
    if (savedUser) { const t = TEACHERS.find(x => x.name === savedUser); if (t) { const isAdm = localStorage.getItem('absensi_admin') === 'true'; const admType = localStorage.getItem('absensi_admin_type') || ''; doLogin(savedUser, isAdm, admType); } }

    document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible' && currentUser) { updateClock(); renderMySchedule(); } });
    updateOfflineStatus();
});

window.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => { updateNextClassInfo(); updateNextClassTimer(); }, 100);
});

(function() {
    const userName = document.getElementById('userNameDisplay');
    if (!userName) return;
    userName.textContent = 'Raka';
    let scrollOffset = 0; let lastScrollY = window.scrollY; let targetOffset = 0;
    function updateFloat() {
        const currentScrollY = window.scrollY; const maxOffset = 80;  
        const rawOffset = -(currentScrollY * 0.08); targetOffset = Math.max(-maxOffset, Math.min(maxOffset, rawOffset));
        scrollOffset += (targetOffset - scrollOffset) * 0.1;
        userName.style.setProperty('--scroll-offset', scrollOffset + 'px');
        userName.classList.add('floating');
        lastScrollY = currentScrollY; requestAnimationFrame(updateFloat);
    }
    updateFloat();
})();
